...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/FieldCacheSanityChecker.java
//
...
- (IOSObjectArray *)checkWithOrgApacheLuceneSearchFieldCache_CacheEntryArray:(IOSObjectArray *)cacheEntries {
  if (nil == cacheEntries || 0 == cacheEntries->size_) return [IOSObjectArray newArrayWithLength:0 type:OrgApacheLuceneUtilFieldCacheSanityChecker_Insanity_class_()];
  if (nil != ramCalc_) {
  ...
  OrgApacheLuceneUtilMapOfSets *valIdToItems = new_OrgApacheLuceneUtilMapOfSets_initWithJavaUtilMap_(new_JavaUtilHashMap_initWithInt_(17));
  OrgApacheLuceneUtilMapOfSets *readerFieldToValIds = new_OrgApacheLuceneUtilMapOfSets_initWithJavaUtilMap_(new_JavaUtilHashMap_initWithInt_(17));
  id<JavaUtilSet> valMismatchKeys = new_JavaUtilHashSet_init();
  for (jint i = 0; i < cacheEntries->size_; i++) {
    ...
    OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField *rf = new_OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField_initWithId_withNSString_([item getReaderKey], [item getFieldName]);
  ...
  id<JavaUtilList> insanity = new_JavaUtilArrayList_initWithInt_([valMismatchKeys size] * 3);
  ...
  return [insanity toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:[insanity size] type:OrgApacheLuceneUtilFieldCacheSanityChecker_Insanity_class_()]];
}
...
IOSObjectArray *OrgApacheLuceneUtilFieldCacheSanityChecker_checkSanityWithOrgApacheLuceneSearchFieldCache_CacheEntryArray_(IOSObjectArray *cacheEntries) {
  OrgApacheLuceneUtilFieldCacheSanityChecker_initialize();
  OrgApacheLuceneUtilFieldCacheSanityChecker *sanityChecker = new_OrgApacheLuceneUtilFieldCacheSanityChecker_init();
  [sanityChecker setRamUsageEstimatorWithOrgApacheLuceneUtilRamUsageEstimator:new_OrgApacheLuceneUtilRamUsageEstimator_initWithBoolean_(false)];
  return [sanityChecker checkWithOrgApacheLuceneSearchFieldCache_CacheEntryArray:cacheEntries];
}

id<JavaUtilCollection> OrgApacheLuceneUtilFieldCacheSanityChecker_checkValueMismatchWithOrgApacheLuceneUtilMapOfSets_withOrgApacheLuceneUtilMapOfSets_withJavaUtilSet_(OrgApacheLuceneUtilFieldCacheSanityChecker *self, OrgApacheLuceneUtilMapOfSets *valIdToItems, OrgApacheLuceneUtilMapOfSets *readerFieldToValIds, id<JavaUtilSet> valMismatchKeys) {
  id<JavaUtilList> insanity = new_JavaUtilArrayList_initWithInt_([((id<JavaUtilSet>) nil_chk(valMismatchKeys)) size] * 3);
  if (![valMismatchKeys isEmpty]) {
    ...
    for (OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField * __strong rf in valMismatchKeys) {
      id<JavaUtilList> badEntries = new_JavaUtilArrayList_initWithInt_([valMismatchKeys size] * 2);
      ...
      IOSObjectArray *badness = [IOSObjectArray newArrayWithLength:[badEntries size] type:OrgApacheLuceneSearchFieldCache_CacheEntry_class_()];
      badness = [badEntries toArrayWithNSObjectArray:badness];
      [insanity addWithId:new_OrgApacheLuceneUtilFieldCacheSanityChecker_Insanity_initWithOrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_withNSString_withOrgApacheLuceneSearchFieldCache_CacheEntryArray_(JreLoadStatic(OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType, VALUEMISMATCH), JreStrcat("$$", @"Multiple distinct value objects for ", [((OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField *) nil_chk(rf)) description]), badness)];
    }
  }
  return insanity;
}
...
id<JavaUtilCollection> OrgApacheLuceneUtilFieldCacheSanityChecker_checkSubreadersWithOrgApacheLuceneUtilMapOfSets_withOrgApacheLuceneUtilMapOfSets_(OrgApacheLuceneUtilFieldCacheSanityChecker *self, OrgApacheLuceneUtilMapOfSets *valIdToItems, OrgApacheLuceneUtilMapOfSets *readerFieldToValIds) {
  id<JavaUtilList> insanity = new_JavaUtilArrayList_initWithInt_(23);
  id<JavaUtilMap> badChildren = new_JavaUtilHashMap_initWithInt_(17);
  OrgApacheLuceneUtilMapOfSets *badKids = new_OrgApacheLuceneUtilMapOfSets_initWithJavaUtilMap_(badChildren);
  ...
  id<JavaUtilSet> seen = new_JavaUtilHashSet_initWithInt_(17);
  ...
  for (OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField * __strong rf in nil_chk(readerFields)) {
    ...
    for (id __strong kidKey in nil_chk(kids)) {
      OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField *kid = new_OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField_initWithId_withNSString_(kidKey, rf->fieldName_);
      if ([badChildren containsKeyWithId:kid]) {
        ...
        (void) [badChildren removeWithId:kid];
      }
  ...
  for (OrgApacheLuceneUtilFieldCacheSanityChecker_ReaderField * __strong parent in nil_chk([badChildren keySet])) {
    ...
    id<JavaUtilList> badEntries = new_JavaUtilArrayList_initWithInt_([((id<JavaUtilSet>) nil_chk(kids)) size] * 2);
    {
...
id<JavaUtilList> OrgApacheLuceneUtilFieldCacheSanityChecker_getAllDescendantReaderKeysWithId_(OrgApacheLuceneUtilFieldCacheSanityChecker *self, id seed) {
  id<JavaUtilList> all = new_JavaUtilArrayList_initWithInt_(17);
  [all addWithId:seed];
...
- (NSString *)description {
  ...
  (void) [((JavaLangStringBuilder *) nil_chk([buf appendWithId:[self getType]])) appendWithNSString:@": "];
  NSString *m = [self getMsg];
  if (nil != m) (void) [buf appendWithNSString:m];
  (void) [buf appendWithChar:0x000a];
  IOSObjectArray *ce = [self getCacheEntries];
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(ce))->size_; i++) {
    (void) [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([buf appendWithChar:0x0009])) appendWithNSString:[((OrgApacheLuceneSearchFieldCache_CacheEntry *) nil_chk(IOSObjectArray_Get(ce, i))) description]])) appendWithChar:0x000a];
  }
...
void OrgApacheLuceneUtilFieldCacheSanityChecker_Insanity_initWithOrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_withNSString_withOrgApacheLuceneSearchFieldCache_CacheEntryArray_(OrgApacheLuceneUtilFieldCacheSanityChecker_Insanity *self, OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType *type, NSString *msg, IOSObjectArray *entries) {
  NSObject_init(self);
  if (nil == type) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Insanity requires non-null InsanityType");
  }
  if (nil == entries || 0 == entries->size_) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Insanity requires non-null/non-empty CacheEntry[]");
  }
  self->type_ = type;
  self->msg_ = msg;
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType class]) {
    OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_SUBREADER = new_OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_initWithNSString_(@"SUBREADER");
    OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_VALUEMISMATCH = new_OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_initWithNSString_(@"VALUEMISMATCH");
    OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_EXPECTED = new_OrgApacheLuceneUtilFieldCacheSanityChecker_InsanityType_initWithNSString_(@"EXPECTED");
