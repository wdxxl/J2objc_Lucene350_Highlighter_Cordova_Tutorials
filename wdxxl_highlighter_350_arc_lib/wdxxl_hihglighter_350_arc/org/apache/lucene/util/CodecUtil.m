...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/CodecUtil.java
//
...
OrgApacheLuceneStoreDataOutput *OrgApacheLuceneUtilCodecUtil_writeHeaderWithOrgApacheLuceneStoreDataOutput_withNSString_withInt_(OrgApacheLuceneStoreDataOutput *outArg, NSString *codec, jint version_) {
  ...
  OrgApacheLuceneUtilBytesRef *bytes = new_OrgApacheLuceneUtilBytesRef_initWithJavaLangCharSequence_(codec);
  if (bytes->length_ != [((NSString *) nil_chk(codec)) java_length] || bytes->length_ >= 128) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$C", @"codec must be simple ASCII, less than 128 characters in length [got ", codec, ']'));
  }
...
jint OrgApacheLuceneUtilCodecUtil_checkHeaderWithOrgApacheLuceneStoreDataInput_withNSString_withInt_withInt_(OrgApacheLuceneStoreDataInput *inArg, NSString *codec, jint minVersion, jint maxVersion) {
  ...
  if (actualHeader != OrgApacheLuceneUtilCodecUtil_CODEC_MAGIC) {
    @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$I$I$@C", @"codec header mismatch: actual header=", actualHeader, @" vs expected header=", OrgApacheLuceneUtilCodecUtil_CODEC_MAGIC, @" (resource: ", inArg, ')'));
  }
  NSString *actualCodec = [inArg readString];
  if (![((NSString *) nil_chk(actualCodec)) isEqual:codec]) {
    @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$$$$$@C", @"codec mismatch: actual codec=", actualCodec, @" vs expected codec=", codec, @" (resource: ", inArg, ')'));
  }
  jint actualVersion = [inArg readInt];
  if (actualVersion < minVersion) {
    @throw new_OrgApacheLuceneIndexIndexFormatTooOldException_initWithOrgApacheLuceneStoreDataInput_withInt_withInt_withInt_(inArg, actualVersion, minVersion, maxVersion);
  }
  if (actualVersion > maxVersion) {
    @throw new_OrgApacheLuceneIndexIndexFormatTooNewException_initWithOrgApacheLuceneStoreDataInput_withInt_withInt_withInt_(inArg, actualVersion, minVersion, maxVersion);
  }
