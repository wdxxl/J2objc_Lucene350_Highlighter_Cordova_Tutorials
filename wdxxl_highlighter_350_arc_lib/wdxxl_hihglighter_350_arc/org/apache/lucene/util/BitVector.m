...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/BitVector.java
//
...
- (id)java_clone {
  IOSByteArray *copyBits = [IOSByteArray newArrayWithLength:((IOSByteArray *) nil_chk(bits_))->size_];
  JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(bits_, 0, copyBits, 0, bits_->size_);
  OrgApacheLuceneUtilBitVector *clone = new_OrgApacheLuceneUtilBitVector_initWithByteArray_withInt_(copyBits, size_);
  clone->count_ = count_;
  return clone;
}
...
- (void)setWithInt:(jint)bit {
  if (bit >= size_) {
    @throw new_JavaLangArrayIndexOutOfBoundsException_initWithNSString_(JreStrcat("$I$I", @"bit=", bit, @" size=", size_));
  }
...
- (void)clearWithInt:(jint)bit {
  if (bit >= size_) {
    @throw new_JavaLangArrayIndexOutOfBoundsException_initWithInt_(bit);
  }
  *IOSByteArray_GetRef(nil_chk(bits_), JreRShift32(bit, 3)) &= ~(JreLShift32(1, (bit & 7)));
  count_ = -1;
}
...
- (void)writeWithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)d
  ...
  @try {
    ...
    (void) OrgApacheLuceneUtilCodecUtil_writeHeaderWithOrgApacheLuceneStoreDataOutput_withNSString_withInt_(output, OrgApacheLuceneUtilBitVector_CODEC, OrgApacheLuceneUtilBitVector_VERSION_CURRENT);
...
void OrgApacheLuceneUtilBitVector_initWithInt_(OrgApacheLuceneUtilBitVector *self, jint n) {
  NSObject_init(self);
  self->size_ = n;
  self->bits_ = [IOSByteArray newArrayWithLength:OrgApacheLuceneUtilBitVector_getNumBytesWithInt_(self, self->size_)];
  self->count_ = 0;
}
...
void OrgApacheLuceneUtilBitVector_readDgapsWithOrgApacheLuceneStoreIndexInput_(OrgApacheLuceneUtilBitVector *self, OrgApacheLuceneStoreIndexInput *input) {
  ...
  self->bits_ = [IOSByteArray newArrayWithLength:(JreRShift32(self->size_, 3)) + 1];
  jint last = 0;
  jint n = OrgApacheLuceneUtilBitVector_count(self);
  while (n > 0) {
    last += [input readVInt];
