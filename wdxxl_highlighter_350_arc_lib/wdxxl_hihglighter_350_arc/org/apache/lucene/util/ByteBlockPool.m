...
- (void)dropBuffersAndReset {
  if (bufferUpto_ != -1) {
    ...
    buffers_ = [IOSObjectArray newArrayWithLength:10 type:IOSClass_byteArray(1)];
    buffer_ = nil;
  }
}
...
- (void)nextBuffer {
  ...
  buffer_ = IOSObjectArray_Set(buffers_, 1 + bufferUpto_, [((OrgApacheLuceneUtilByteBlockPool_Allocator *) nil_chk(allocator_)) getByteBlock]);
  bufferUpto_++;
  byteUpto_ = 0;
  byteOffset_ += OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SIZE;
}
...
- (OrgApacheLuceneUtilBytesRef *)setBytesRefWithOrgApacheLuceneUtilBytesRef:(OrgApacheLuceneUtilBytesRef *)term
  ...
  IOSByteArray *bytes = ((OrgApacheLuceneUtilBytesRef *) nil_chk(term))->bytes_ = IOSObjectArray_Get(nil_chk(buffers_), JreRShift32(textStart, OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SHIFT));
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilByteBlockPool class]) {
    OrgApacheLuceneUtilByteBlockPool_nextLevelArray = [IOSIntArray newArrayWithInts:(jint[]){ 1, 2, 3, 4, 5, 6, 7, 8, 9, 9 } count:10];
    OrgApacheLuceneUtilByteBlockPool_levelSizeArray = [IOSIntArray newArrayWithInts:(jint[]){ 5, 14, 20, 30, 40, 40, 80, 80, 120, 200 } count:10];
...
- (void)recycleByteBlocksWithJavaUtilList:(id<JavaUtilList>)blocks {
  IOSObjectArray *b = [((id<JavaUtilList>) nil_chk(blocks)) toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:[blocks size] type:IOSClass_byteArray(1)]];
