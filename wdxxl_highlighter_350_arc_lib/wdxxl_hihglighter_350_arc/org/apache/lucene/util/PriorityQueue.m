...
- (void)initialize__WithInt:(jint)maxSize {
  ...
  heap_ = [IOSObjectArray newArrayWithLength:heapSize type:NSObject_class_()];
  self->maxSize_ = maxSize;
  id sentinel = [self getSentinelObject];
  if (sentinel != nil) {
    (void) IOSObjectArray_Set(nil_chk(heap_), 1, sentinel);
    for (jint i = 2; i < ((IOSObjectArray *) nil_chk(heap_))->size_; i++) {
      (void) IOSObjectArray_Set(heap_, i, [self getSentinelObject]);
    }
    size_ = maxSize;
  }
}
...
- (id)insertWithOverflowWithId:(id)element {
  if (size_ < maxSize_) {
    (void) OrgApacheLuceneUtilPriorityQueue_addWithId_(self, element);
    return nil;
  }
    ...
    (void) IOSObjectArray_Set(heap_, 1, element);
    (void) OrgApacheLuceneUtilPriorityQueue_updateTop(self);
    return ret;
  }
  else {
    return element;
...
- (id)pop {
  if (size_ > 0) {
    ...
    (void) IOSObjectArray_Set(heap_, 1, IOSObjectArray_Get(heap_, size_));
    (void) IOSObjectArray_Set(heap_, size_, nil);
    size_--;
...
id OrgApacheLuceneUtilPriorityQueue_addWithId_(OrgApacheLuceneUtilPriorityQueue *self, id element) {
  self->size_++;
  (void) IOSObjectArray_Set(nil_chk(self->heap_), self->size_, element);
  OrgApacheLuceneUtilPriorityQueue_upHeap(self);
...
void OrgApacheLuceneUtilPriorityQueue_upHeap(OrgApacheLuceneUtilPriorityQueue *self) {
  ...
  while (j > 0 && [self lessThanWithId:node withId:IOSObjectArray_Get(self->heap_, j)]) {
    (void) IOSObjectArray_Set(nil_chk(self->heap_), i, IOSObjectArray_Get(self->heap_, j));
    i = j;
    j = JreURShift32(j, 1);
  }
  (void) IOSObjectArray_Set(nil_chk(self->heap_), i, node);
}
