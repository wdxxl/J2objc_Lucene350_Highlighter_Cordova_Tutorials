...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/IOUtils.java
//
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilIOUtils class]) {
    OrgApacheLuceneUtilIOUtils_CHARSET_UTF_8 = JavaNioCharsetCharset_forNameWithNSString_(@"UTF-8");
    {
      ...
      OrgApacheLuceneUtilIOUtils_SUPPRESS_METHOD = m;
    }
...
void OrgApacheLuceneUtilIOUtils_addSuppressedWithNSException_withNSException_(NSException *exception, NSException *suppressed) {
  ...
  if (OrgApacheLuceneUtilIOUtils_SUPPRESS_METHOD != nil && exception != nil && suppressed != nil) {
    @try {
      (void) [OrgApacheLuceneUtilIOUtils_SUPPRESS_METHOD invokeWithId:exception withNSObjectArray:[IOSObjectArray newArrayWithObjects:(id[]){ suppressed } count:1 type:NSObject_class_()]];
    }
    @catch (JavaLangException *e) {
    }
  }
...
JavaIoReader *OrgApacheLuceneUtilIOUtils_getDecodingReaderWithJavaIoInputStream_withJavaNioCharsetCharset_(JavaIoInputStream *stream, JavaNioCharsetCharset *charSet) {
  ...
  return new_JavaIoBufferedReader_initWithJavaIoReader_(new_JavaIoInputStreamReader_initWithJavaIoInputStream_withJavaNioCharsetCharsetDecoder_(stream, charSetDecoder));
}
...
JavaIoReader *OrgApacheLuceneUtilIOUtils_getDecodingReaderWithJavaIoFile_withJavaNioCharsetCharset_(JavaIoFile *file, JavaNioCharsetCharset *charSet) {
  ...
  @try {
    stream = new_JavaIoFileInputStream_initWithJavaIoFile_(file);
  ...
  @finally {
    if (!success) {
      OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ stream } count:1 type:JavaIoCloseable_class_()]);
    }
  }
}
