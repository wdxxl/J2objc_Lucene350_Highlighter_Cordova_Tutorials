...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/AttributeImpl.java
//
...
- (void)reflectWithWithOrgApacheLuceneUtilAttributeReflector:(id<OrgApacheLuceneUtilAttributeReflector>)reflector {
  ...
  if ([((JavaUtilLinkedList *) nil_chk(interfaces)) size] != 1) {
    @throw new_JavaLangUnsupportedOperationException_initWithNSString_(JreStrcat("$$", [clazz getName], @" implements more than one Attribute interface, the default reflectWith() implementation cannot handle this."));
  }
  ...
  if (enableBackwards_ && [((OrgApacheLuceneUtilVirtualMethod *) nil_chk(OrgApacheLuceneUtilAttributeImpl_toStringMethod)) isOverriddenAsOfWithIOSClass:clazz]) {
    JreAssert((OrgApacheLuceneUtilAttributeImpl_assertExternalClassWithIOSClass_(self, clazz)), (@"no Lucene/Solr classes should fallback to toString() parsing"));
    {
      ...
      while (b__ < e__) {
        ...
        if (pos < 0) {
          @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"The backwards compatibility layer to support reflectWith() on old AtributeImpls expects the toString() implementation to return a correct format as specified for method reflectAsString(false)");
        }
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilAttributeImpl class]) {
    OrgApacheLuceneUtilAttributeImpl_toStringMethod = new_OrgApacheLuceneUtilVirtualMethod_initWithIOSClass_withNSString_withIOSClassArray_(OrgApacheLuceneUtilAttributeImpl_class_(), @"toString", [IOSObjectArray newArrayWithLength:0 type:IOSClass_class_()]);
...
NSString *OrgApacheLuceneUtilAttributeImpl_reflectAsStringWithBoolean_(OrgApacheLuceneUtilAttributeImpl *self, jboolean prependAttClass) {
  ...
  [self reflectWithWithOrgApacheLuceneUtilAttributeReflector:new_OrgApacheLuceneUtilAttributeImpl_1_initWithJavaLangStringBuilder_withBoolean_(buffer, prependAttClass)];
