...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/messages/MessageImpl.java
//
...
- (NSString *)description {
  ...
  JavaLangStringBuilder *sb = new_JavaLangStringBuilder_initWithNSString_([self getKey]);
  if (args != nil) {
    for (jint i = 0; i < args->size_; i++) {
      (void) [((JavaLangStringBuilder *) nil_chk([sb appendWithNSString:i == 0 ? @" " : @", "])) appendWithId:IOSObjectArray_Get(args, i)];
    }
  }
...
void OrgApacheLuceneMessagesMessageImpl_initWithNSString_(OrgApacheLuceneMessagesMessageImpl *self, NSString *key) {
  NSObject_init(self);
  self->arguments_ = [IOSObjectArray newArrayWithLength:0 type:NSObject_class_()];
  self->key_ = key;
}
