...
- (jfloat)phraseFreq {
  ...
  while ([pp nextPosition] && (end = OrgApacheLuceneSearchSloppyPhraseScorer_advanceRepeatsWithOrgApacheLuceneSearchPhrasePositions_withInt_(self, pp, end)) != JavaLangInteger_MIN_VALUE) {
    if (pp->position_ > next) {
      ...
      (void) [((OrgApacheLuceneSearchPhraseQueue *) nil_chk(pq_)) addWithId:pp];
...
jint OrgApacheLuceneSearchSloppyPhraseScorer_initPhrasePositions(OrgApacheLuceneSearchSloppyPhraseScorer *self) {
  ...
  if (self->checkedRepeats_ && !self->hasRepeats_) {
    ...
    for (OrgApacheLuceneSearchPhrasePositions *pp = self->min_, *prev = nil; prev != self->max_; pp = (prev = pp)->next_) {
      ...
      (void) [((OrgApacheLuceneSearchPhraseQueue *) nil_chk(self->pq_)) addWithId:pp];
    }
    return end;
  }
  for (OrgApacheLuceneSearchPhrasePositions *pp = self->min_, *prev = nil; prev != self->max_; pp = (prev = pp)->next_) {
  ...
  if (!self->checkedRepeats_) {
    ...
    JavaUtilArrayList *ppsA = new_JavaUtilArrayList_init();
    OrgApacheLuceneSearchPhrasePositions *dummyPP = new_OrgApacheLuceneSearchPhrasePositions_initWithOrgApacheLuceneIndexTermPositions_withInt_withInt_(nil, -1, -1);
    for (OrgApacheLuceneSearchPhrasePositions *pp = self->min_, *prev = nil; prev != self->max_; pp = (prev = pp)->next_) {
      ...
      for (OrgApacheLuceneSearchPhrasePositions *prevB = pp, *pp2 = pp->next_; pp2 != self->min_; pp2 = pp2->next_) {
        ...
        prevB->nextRepeating_ = pp2;
        pp2->nextRepeating_ = dummyPP;
        prevB = pp2;
      }
    }
    if (self->hasRepeats_) {
      for (OrgApacheLuceneSearchPhrasePositions *pp = self->min_, *prev = nil; prev != self->max_; pp = (prev = pp)->next_) {
        if (((OrgApacheLuceneSearchPhrasePositions *) nil_chk(pp))->nextRepeating_ == dummyPP) {
          pp->nextRepeating_ = nil;
        }
      }
    }
    self->nrPps_ = [ppsA toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:0 type:OrgApacheLuceneSearchPhrasePositions_class_()]];
    self->pq_ = new_OrgApacheLuceneSearchPhraseQueue_initWithInt_(((IOSObjectArray *) nil_chk(self->nrPps_))->size_);
  }
  if (self->hasRepeats_) {
    {
