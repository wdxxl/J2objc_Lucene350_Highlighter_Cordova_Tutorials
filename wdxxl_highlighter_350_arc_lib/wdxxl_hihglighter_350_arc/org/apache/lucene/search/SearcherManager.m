...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/SearcherManager.java
//
...
- (jboolean)maybeReopen {
  ...
  if ([((JavaUtilConcurrentSemaphore *) nil_chk(reopenLock_)) tryAcquire]) {
    @try {
      ...
      if (newReader != nil) {
        OrgApacheLuceneSearchIndexSearcher *newSearcher = new_OrgApacheLuceneSearchIndexSearcher_initWithOrgApacheLuceneIndexIndexReader_withJavaUtilConcurrentExecutorService_(newReader, es_);
...
- (OrgApacheLuceneSearchIndexSearcher *)acquire {
  ...
  do {
    if ((searcher = JreLoadVolatileId(&currentSearcher_)) == nil) {
      @throw new_OrgApacheLuceneStoreAlreadyClosedException_initWithNSString_(@"this SearcherManager is closed");
    }
  }
...
void OrgApacheLuceneSearchSearcherManager_initWithOrgApacheLuceneIndexIndexWriter_withBoolean_withOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(OrgApacheLuceneSearchSearcherManager *self, OrgApacheLuceneIndexIndexWriter *writer, jboolean applyAllDeletes, id<OrgApacheLuceneSearchSearcherWarmer> warmer, id<JavaUtilConcurrentExecutorService> es) {
  NSObject_init(self);
  self->reopenLock_ = new_JavaUtilConcurrentSemaphore_initWithInt_(1);
  self->es_ = es;
  self->warmer_ = warmer;
  (void) JreVolatileStrongAssign(&self->currentSearcher_, new_OrgApacheLuceneSearchIndexSearcher_initWithOrgApacheLuceneIndexIndexReader_(OrgApacheLuceneIndexIndexReader_openWithOrgApacheLuceneIndexIndexWriter_withBoolean_(writer, applyAllDeletes)));
  if (warmer != nil) {
    (void) [((OrgApacheLuceneIndexIndexWriterConfig *) nil_chk([((OrgApacheLuceneIndexIndexWriter *) nil_chk(writer)) getConfig])) setMergedSegmentWarmerWithOrgApacheLuceneIndexIndexWriter_IndexReaderWarmer:new_OrgApacheLuceneSearchSearcherManager_1_initWithOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(warmer, es)];
  }
}
...
void OrgApacheLuceneSearchSearcherManager_initWithOrgApacheLuceneStoreDirectory_withOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(OrgApacheLuceneSearchSearcherManager *self, OrgApacheLuceneStoreDirectory *dir, id<OrgApacheLuceneSearchSearcherWarmer> warmer, id<JavaUtilConcurrentExecutorService> es) {
  ...
  self->es_ = es;
  self->warmer_ = warmer;
  (void) JreVolatileStrongAssign(&self->currentSearcher_, new_OrgApacheLuceneSearchIndexSearcher_initWithOrgApacheLuceneIndexIndexReader_withJavaUtilConcurrentExecutorService_(OrgApacheLuceneIndexIndexReader_openWithOrgApacheLuceneStoreDirectory_withBoolean_(dir, true), es));
}
...
void OrgApacheLuceneSearchSearcherManager_swapSearcherWithOrgApacheLuceneSearchIndexSearcher_(OrgApacheLuceneSearchSearcherManager *self, OrgApacheLuceneSearchIndexSearcher *newSearcher) {
  @synchronized(self) {
    ...
    (void) JreVolatileStrongAssign(&self->currentSearcher_, newSearcher);
...
- (void)warmWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader {
  [((id<OrgApacheLuceneSearchSearcherWarmer>) nil_chk(val$warmer_)) warmWithOrgApacheLuceneSearchIndexSearcher:new_OrgApacheLuceneSearchIndexSearcher_initWithOrgApacheLuceneIndexIndexReader_withJavaUtilConcurrentExecutorService_(reader, val$es_)];
}
...
void OrgApacheLuceneSearchSearcherManager_1_initWithOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(OrgApacheLuceneSearchSearcherManager_1 *self, id<OrgApacheLuceneSearchSearcherWarmer> capture$0, id<JavaUtilConcurrentExecutorService> capture$1) {
  self->val$warmer_ = capture$0;
  self->val$es_ = capture$1;
