...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/TimeLimitingCollector.java
//
...
- (void)collectWithInt:(jint)doc {
  ...
  if (timeout_ < time) {
    ...
    @throw new_OrgApacheLuceneSearchTimeLimitingCollector_TimeExceededException_initWithLong_withLong_withInt_(timeout_ - t0_, time - t0_, docBase_ + doc);
  }
  [((OrgApacheLuceneSearchCollector *) nil_chk(collector_)) collectWithInt:doc];
}
...
+ (void)initialize {
  if (self == [OrgApacheLuceneSearchTimeLimitingCollector_TimerThreadHolder class]) {
    {
      OrgApacheLuceneSearchTimeLimitingCollector_TimerThreadHolder_THREAD = new_OrgApacheLuceneSearchTimeLimitingCollector_TimerThread_initWithOrgApacheLuceneUtilCounter_(OrgApacheLuceneUtilCounter_newCounterWithBoolean_(true));
