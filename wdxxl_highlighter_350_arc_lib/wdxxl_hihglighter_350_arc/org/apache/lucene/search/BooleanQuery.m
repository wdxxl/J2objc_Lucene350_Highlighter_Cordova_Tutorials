...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/BooleanQuery.java
//
...
withOrgApacheLuceneSearchBooleanClause_Occur:(OrgApacheLuceneSearchBooleanClause_Occur *)occur {
  [self addWithOrgApacheLuceneSearchBooleanClause:new_OrgApacheLuceneSearchBooleanClause_initWithOrgApacheLuceneSearchQuery_withOrgApacheLuceneSearchBooleanClause_Occur_(query, occur)];
}

- (void)addWithOrgApacheLuceneSearchBooleanClause:(OrgApacheLuceneSearchBooleanClause *)clause {
  if ([((JavaUtilArrayList *) nil_chk(clauses_)) size] >= OrgApacheLuceneSearchBooleanQuery_maxClauseCount) @throw new_OrgApacheLuceneSearchBooleanQuery_TooManyClauses_init();
  [((JavaUtilArrayList *) nil_chk(clauses_)) addWithId:clause];
}

- (IOSObjectArray *)getClauses {
  return [((JavaUtilArrayList *) nil_chk(clauses_)) toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:[clauses_ size] type:OrgApacheLuceneSearchBooleanClause_class_()]];
}
...
- (OrgApacheLuceneSearchWeight *)createWeightWithOrgApacheLuceneSearchSearcher:(OrgApacheLuceneSearchSearcher *)searcher {
  return new_OrgApacheLuceneSearchBooleanQuery_BooleanWeight_initWithOrgApacheLuceneSearchBooleanQuery_withOrgApacheLuceneSearchSearcher_withBoolean_(self, searcher, disableCoord_);
}
...
- (OrgApacheLuceneSearchQuery *)rewriteWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader {
  ...
  for (jint i = 0; i < [((JavaUtilArrayList *) nil_chk(clauses_)) size]; i++) {
    ...
    if (query != [c getQuery]) {
      ...
      (void) [((JavaUtilArrayList *) nil_chk(((OrgApacheLuceneSearchBooleanQuery *) nil_chk(clone))->clauses_)) setWithInt:i withId:new_OrgApacheLuceneSearchBooleanClause_initWithOrgApacheLuceneSearchQuery_withOrgApacheLuceneSearchBooleanClause_Occur_(query, [c getOccur])];
    }
  }
  if (clone != nil) {
    return clone;
...
- (id)java_clone {
  ...
  ((OrgApacheLuceneSearchBooleanQuery *) nil_chk(clone))->clauses_ = (JavaUtilArrayList *) cast_chk([((JavaUtilArrayList *) nil_chk(self->clauses_)) java_clone], [JavaUtilArrayList class]);
  return clone;
}
...
- (NSString *)toStringWithNSString:(NSString *)field {
  ...
  for (jint i = 0; i < [((JavaUtilArrayList *) nil_chk(clauses_)) size]; i++) {
    OrgApacheLuceneSearchBooleanClause *c = [((JavaUtilArrayList *) nil_chk(clauses_)) getWithInt:i];
    if ([((OrgApacheLuceneSearchBooleanClause *) nil_chk(c)) isProhibited]) (void) [buffer appendWithNSString:@"-"];
    else if ([c isRequired]) (void) [buffer appendWithNSString:@"+"];
    OrgApacheLuceneSearchQuery *subQuery = [c getQuery];
    if (subQuery != nil) {
      if ([subQuery isKindOfClass:[OrgApacheLuceneSearchBooleanQuery class]]) {
        (void) [buffer appendWithNSString:@"("];
        (void) [buffer appendWithNSString:[subQuery toStringWithNSString:field]];
        (void) [buffer appendWithNSString:@")"];
      }
      else {
        (void) [buffer appendWithNSString:[subQuery toStringWithNSString:field]];
      ...
      (void) [buffer appendWithNSString:@"null"];
    }
    if (i != [((JavaUtilArrayList *) nil_chk(clauses_)) size] - 1) (void) [buffer appendWithNSString:@" "];
  }
  if (needParens) {
    (void) [buffer appendWithNSString:@")"];
  }
  if ([self getMinimumNumberShouldMatch] > 0) {
    ...
    (void) [buffer appendWithInt:[self getMinimumNumberShouldMatch]];
  }
  if ([self getBoost] != 1.0f) {
...
void OrgApacheLuceneSearchBooleanQuery_setMaxClauseCountWithInt_(jint maxClauseCount) {
  OrgApacheLuceneSearchBooleanQuery_initialize();
  if (maxClauseCount < 1) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"maxClauseCount must be >= 1");
...
- (OrgApacheLuceneSearchExplanation *)explainWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader
  ...
  OrgApacheLuceneSearchComplexExplanation *sumExpl = new_OrgApacheLuceneSearchComplexExplanation_init();
  ...
  for (id<JavaUtilIterator> wIter = [((JavaUtilArrayList *) nil_chk(weights_)) iterator]; [((id<JavaUtilIterator>) nil_chk(wIter)) hasNext]; ) {
    ...
    if ([((OrgApacheLuceneSearchWeight *) nil_chk(w)) scorerWithOrgApacheLuceneIndexIndexReader:reader withBoolean:true withBoolean:true] == nil) {
      if ([((OrgApacheLuceneSearchBooleanClause *) nil_chk(c)) isRequired]) {
        fail = true;
        OrgApacheLuceneSearchExplanation *r = new_OrgApacheLuceneSearchExplanation_initWithFloat_withNSString_(0.0f, JreStrcat("$$C", @"no match on required clause (", [((OrgApacheLuceneSearchQuery *) nil_chk([c getQuery])) description], ')'));
    ...
    if ([((OrgApacheLuceneSearchExplanation *) nil_chk(e)) isMatch]) {
      if (![((OrgApacheLuceneSearchBooleanClause *) nil_chk(c)) isProhibited]) {
        ...
        OrgApacheLuceneSearchExplanation *r = new_OrgApacheLuceneSearchExplanation_initWithFloat_withNSString_(0.0f, JreStrcat("$$C", @"match on prohibited clause (", [((OrgApacheLuceneSearchQuery *) nil_chk([c getQuery])) description], ')'));
  ...
  if (coordFactor == 1.0f) {
    ...
    OrgApacheLuceneSearchComplexExplanation *result = new_OrgApacheLuceneSearchComplexExplanation_initWithBoolean_withFloat_withNSString_([sumExpl isMatch], sum * coordFactor, @"product of:");
    [result addDetailWithOrgApacheLuceneSearchExplanation:sumExpl];
    [result addDetailWithOrgApacheLuceneSearchExplanation:new_OrgApacheLuceneSearchExplanation_initWithFloat_withNSString_(coordFactor, JreStrcat("$ICIC", @"coord(", coord, '/', maxCoord_, ')'))];
    return result;
  }
}
...
- (OrgApacheLuceneSearchScorer *)scorerWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader
  ...
  id<JavaUtilList> required = new_JavaUtilArrayList_init();
  id<JavaUtilList> prohibited = new_JavaUtilArrayList_init();
  id<JavaUtilList> optional = new_JavaUtilArrayList_init();
  id<JavaUtilIterator> cIter = [((JavaUtilArrayList *) nil_chk(this$0_->clauses_)) iterator];
  for (OrgApacheLuceneSearchWeight * __strong w in nil_chk(weights_)) {
    OrgApacheLuceneSearchBooleanClause *c = [((id<JavaUtilIterator>) nil_chk(cIter)) next];
  ...
  if (!scoreDocsInOrder && topScorer && [required size] == 0) {
    return new_OrgApacheLuceneSearchBooleanScorer_initWithOrgApacheLuceneSearchWeight_withBoolean_withOrgApacheLuceneSearchSimilarity_withInt_withJavaUtilList_withJavaUtilList_withInt_(self, disableCoord_, similarity_, this$0_->minNrShouldMatch_, optional, prohibited, maxCoord_);
  }
  ...
  return new_OrgApacheLuceneSearchBooleanScorer2_initWithOrgApacheLuceneSearchWeight_withBoolean_withOrgApacheLuceneSearchSimilarity_withInt_withJavaUtilList_withJavaUtilList_withJavaUtilList_withInt_(self, disableCoord_, similarity_, this$0_->minNrShouldMatch_, required, prohibited, optional, maxCoord_);
}
...
void OrgApacheLuceneSearchBooleanQuery_BooleanWeight_initWithOrgApacheLuceneSearchBooleanQuery_withOrgApacheLuceneSearchSearcher_withBoolean_(OrgApacheLuceneSearchBooleanQuery_BooleanWeight *self, OrgApacheLuceneSearchBooleanQuery *outer$, OrgApacheLuceneSearchSearcher *searcher, jboolean disableCoord) {
  ...
  self->disableCoord_ = disableCoord;
  self->weights_ = new_JavaUtilArrayList_initWithInt_([((JavaUtilArrayList *) nil_chk(outer$->clauses_)) size]);
