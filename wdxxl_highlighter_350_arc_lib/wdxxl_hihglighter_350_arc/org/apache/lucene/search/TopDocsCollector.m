...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/TopDocsCollector.java
//
...
- (void)populateResultsWithOrgApacheLuceneSearchScoreDocArray:(IOSObjectArray *)results
  ...
  for (jint i = howMany - 1; i >= 0; i--) {
    (void) IOSObjectArray_Set(nil_chk(results), i, [((OrgApacheLuceneUtilPriorityQueue *) nil_chk(pq_)) pop]);
  }
}

- (OrgApacheLuceneSearchTopDocs *)newTopDocsWithOrgApacheLuceneSearchScoreDocArray:(IOSObjectArray *)results
                                                                           withInt:(jint)start {
  return results == nil ? OrgApacheLuceneSearchTopDocsCollector_EMPTY_TOPDOCS : new_OrgApacheLuceneSearchTopDocs_initWithInt_withOrgApacheLuceneSearchScoreDocArray_(totalHits_, results);
}
...
- (OrgApacheLuceneSearchTopDocs *)topDocsWithInt:(jint)start
  ...
  IOSObjectArray *results = [IOSObjectArray newArrayWithLength:howMany type:OrgApacheLuceneSearchScoreDoc_class_()];
...
+ (void)initialize {
  if (self == [OrgApacheLuceneSearchTopDocsCollector class]) {
    OrgApacheLuceneSearchTopDocsCollector_EMPTY_TOPDOCS = new_OrgApacheLuceneSearchTopDocs_initWithInt_withOrgApacheLuceneSearchScoreDocArray_withFloat_(0, [IOSObjectArray newArrayWithLength:0 type:OrgApacheLuceneSearchScoreDoc_class_()], JavaLangFloat_NaN);
