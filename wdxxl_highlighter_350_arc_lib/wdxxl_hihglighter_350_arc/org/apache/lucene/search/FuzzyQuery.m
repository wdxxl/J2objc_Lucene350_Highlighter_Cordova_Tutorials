...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/FuzzyQuery.java
//
...
- (OrgApacheLuceneSearchFilteredTermEnum *)getEnumWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader {
  if (!termLongEnough_) {
    return new_OrgApacheLuceneSearchSingleTermEnum_initWithOrgApacheLuceneIndexIndexReader_withOrgApacheLuceneIndexTerm_(reader, term_);
  }
  return new_OrgApacheLuceneSearchFuzzyTermEnum_initWithOrgApacheLuceneIndexIndexReader_withOrgApacheLuceneIndexTerm_withFloat_withInt_(reader, [self getTerm], minimumSimilarity_, prefixLength_);
}
...
- (NSString *)toStringWithNSString:(NSString *)field {
  ...
  (void) [buffer appendWithNSString:JavaLangFloat_toStringWithFloat_(minimumSimilarity_)];
...
void OrgApacheLuceneSearchFuzzyQuery_initWithOrgApacheLuceneIndexTerm_withFloat_withInt_withInt_(OrgApacheLuceneSearchFuzzyQuery *self, OrgApacheLuceneIndexTerm *term, jfloat minimumSimilarity, jint prefixLength, jint maxExpansions) {
  ...
  self->term_ = term;
  if (minimumSimilarity >= 1.0f) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"minimumSimilarity >= 1");
  else if (minimumSimilarity < 0.0f) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"minimumSimilarity < 0");
  if (prefixLength < 0) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"prefixLength < 0");
  if (maxExpansions < 0) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"maxExpansions < 0");
  [self setRewriteMethodWithOrgApacheLuceneSearchMultiTermQuery_RewriteMethod:new_OrgApacheLuceneSearchMultiTermQuery_TopTermsScoringBooleanQueryRewrite_initWithInt_(maxExpansions)];
