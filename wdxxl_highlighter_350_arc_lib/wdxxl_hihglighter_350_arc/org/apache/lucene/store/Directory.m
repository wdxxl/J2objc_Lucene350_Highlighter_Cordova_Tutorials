...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/Directory.java
//
...
- (void)copy__WithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)to
  ...
  @finally {
    OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaLangException_withJavaIoCloseableArray_(priorException, [IOSObjectArray newArrayWithObjects:(id[]){ os, is } count:2 type:JavaIoCloseable_class_()]);
  }
}
...
- (void)ensureOpen {
  if (!JreLoadVolatileBoolean(&isOpen_)) @throw new_OrgApacheLuceneStoreAlreadyClosedException_initWithNSString_(@"this Directory is closed");
}
