...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/SimpleFSLockFactory.java
//
...
- (OrgApacheLuceneStoreLock *)makeLockWithNSString:(NSString *)lockName {
  ...
  return new_OrgApacheLuceneStoreSimpleFSLock_initWithJavaIoFile_withNSString_(lockDir_, lockName);
}
...
- (void)clearLockWithNSString:(NSString *)lockName {
  if ([((JavaIoFile *) nil_chk(lockDir_)) exists]) {
    ...
    JavaIoFile *lockFile = new_JavaIoFile_initWithJavaIoFile_withNSString_(lockDir_, lockName);
    if ([lockFile exists] && ![lockFile delete__]) {
      @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"Cannot delete ", lockFile));
    }
  }
}
...
void OrgApacheLuceneStoreSimpleFSLockFactory_initWithNSString_(OrgApacheLuceneStoreSimpleFSLockFactory *self, NSString *lockDirName) {
  OrgApacheLuceneStoreFSLockFactory_init(self);
  [self setLockDirWithJavaIoFile:new_JavaIoFile_initWithNSString_(lockDirName)];
}
...
- (void)release__ {
  if ([((JavaIoFile *) nil_chk(lockFile_)) exists] && ![((JavaIoFile *) nil_chk(lockFile_)) delete__]) @throw new_OrgApacheLuceneStoreLockReleaseFailedException_initWithNSString_(JreStrcat("$@", @"failed to delete ", lockFile_));
}
...
void OrgApacheLuceneStoreSimpleFSLock_initWithJavaIoFile_withNSString_(OrgApacheLuceneStoreSimpleFSLock *self, JavaIoFile *lockDir, NSString *lockFileName) {
  ...
  self->lockFile_ = new_JavaIoFile_initWithJavaIoFile_withNSString_(lockDir, lockFileName);
}
