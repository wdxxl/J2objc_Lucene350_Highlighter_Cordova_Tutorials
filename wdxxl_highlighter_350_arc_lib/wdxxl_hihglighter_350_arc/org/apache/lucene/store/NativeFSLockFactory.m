...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/NativeFSLockFactory.java
//
...
- (OrgApacheLuceneStoreLock *)makeLockWithNSString:(NSString *)lockName {
  @synchronized(self) {
    ...
    return new_OrgApacheLuceneStoreNativeFSLock_initWithJavaIoFile_withNSString_(lockDir_, lockName);
  }
}
...
- (void)clearLockWithNSString:(NSString *)lockName {
  if ([((JavaIoFile *) nil_chk(lockDir_)) exists]) {
    ...
    [new_JavaIoFile_initWithJavaIoFile_withNSString_(lockDir_, lockName) delete__];
  }
}
...
void OrgApacheLuceneStoreNativeFSLockFactory_initWithNSString_(OrgApacheLuceneStoreNativeFSLockFactory *self, NSString *lockDirName) {
  OrgApacheLuceneStoreNativeFSLockFactory_initWithJavaIoFile_(self, new_JavaIoFile_initWithNSString_(lockDirName));
}
...
- (jboolean)obtain {
  @synchronized(self) {
    ...
    if (![((JavaIoFile *) nil_chk(lockDir_)) exists]) {
      if (![((JavaIoFile *) nil_chk(lockDir_)) mkdirs]) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$$", @"Cannot create directory: ", [((JavaIoFile *) nil_chk(lockDir_)) getAbsolutePath]));
    }
    else if (![((JavaIoFile *) nil_chk(lockDir_)) isDirectory]) {
      @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$$", @"Found regular file where directory expected: ", [((JavaIoFile *) nil_chk(lockDir_)) getAbsolutePath]));
    }
    ...
    @try {
      ...
      @try {
        f_ = new_JavaIoRandomAccessFile_initWithJavaIoFile_withNSString_(path_, @"rw");
      }
      ...
      if (f_ != nil) {
        @try {
          channel_ = [f_ getChannel];
          @try {
            lock_ = [((JavaNioChannelsFileChannel *) nil_chk(channel_)) tryLock];
          }
          @catch (JavaIoIOException *e) {
            failureReason_ = e;
          }
    ...
    if (OrgApacheLuceneStoreNativeFSLock_lockExists(self)) {
      ...
      @try {
        if (!(obtained = [self obtain])) {
          @throw new_OrgApacheLuceneStoreLockReleaseFailedException_initWithNSString_(JreStrcat("$@", @"Cannot forcefully unlock a NativeFSLock which is held by another indexer component: ", path_));
        }
      }
      @finally {
        if (obtained) {
...
+ (void)initialize {
  if (self == [OrgApacheLuceneStoreNativeFSLock class]) {
    OrgApacheLuceneStoreNativeFSLock_LOCK_HELD = new_JavaUtilHashSet_init();
...
void OrgApacheLuceneStoreNativeFSLock_initWithJavaIoFile_withNSString_(OrgApacheLuceneStoreNativeFSLock *self, JavaIoFile *lockDir, NSString *lockFileName) {
  ...
  self->path_ = new_JavaIoFile_initWithJavaIoFile_withNSString_(lockDir, lockFileName);
}
