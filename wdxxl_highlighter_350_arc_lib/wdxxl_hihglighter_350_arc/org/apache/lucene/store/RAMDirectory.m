...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/RAMDirectory.java
//
...
- (IOSObjectArray *)listAll {
  ...
  id<JavaUtilList> names = new_JavaUtilArrayList_initWithInt_([((id<JavaUtilSet>) nil_chk(fileNames)) size]);
  for (NSString * __strong name in fileNames) [names addWithId:name];
  return [names toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:[names size] type:NSString_class_()]];
}
...
- (OrgApacheLuceneStoreIndexOutput *)createOutputWithNSString:(NSString *)name {
  ...
  if (existing != nil) {
    ...
    existing->directory_ = nil;
  }
  (void) [fileMap_ putWithId:name withId:file];
  return new_OrgApacheLuceneStoreRAMOutputStream_initWithOrgApacheLuceneStoreRAMFile_(file);
}

- (OrgApacheLuceneStoreRAMFile *)newRAMFile {
  return new_OrgApacheLuceneStoreRAMFile_initWithOrgApacheLuceneStoreRAMDirectory_(self);
}
...
- (OrgApacheLuceneStoreIndexInput *)openInputWithNSString:(NSString *)name {
  ...
  return new_OrgApacheLuceneStoreRAMInputStream_initWithNSString_withOrgApacheLuceneStoreRAMFile_(name, file);
}
...
void OrgApacheLuceneStoreRAMDirectory_init(OrgApacheLuceneStoreRAMDirectory *self) {
  ...
  self->fileMap_ = new_JavaUtilConcurrentConcurrentHashMap_init();
  self->sizeInBytes_ = new_JavaUtilConcurrentAtomicAtomicLong_init();
  @try {
    [self setLockFactoryWithOrgApacheLuceneStoreLockFactory:new_OrgApacheLuceneStoreSingleInstanceLockFactory_init()];
  }
