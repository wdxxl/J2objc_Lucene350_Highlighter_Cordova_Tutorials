...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/queryParser/FastCharStream.java
//
...
- (IOSCharArray *)GetSuffixWithInt:(jint)len {
  IOSCharArray *value = [IOSCharArray newArrayWithLength:len];
...
void OrgApacheLuceneQueryParserFastCharStream_refill(OrgApacheLuceneQueryParserFastCharStream *self) {
  ...
  if (self->tokenStart_ == 0) {
    if (self->buffer_ == nil) {
      self->buffer_ = [IOSCharArray newArrayWithLength:2048];
    }
    else if (self->bufferLength_ == self->buffer_->size_) {
      IOSCharArray *newBuffer = [IOSCharArray newArrayWithLength:self->buffer_->size_ * 2];
  ...
  if (charsRead == -1) @throw new_JavaIoIOException_initWithNSString_(@"read past eof");
