...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/analysis/NormalizeCharMap.java
//
...
- (void)addWithNSString:(NSString *)singleMatch
  ...
  for (jint i = 0; i < [((NSString *) nil_chk(singleMatch)) java_length]; i++) {
    ...
    if (currMap->submap_ == nil) {
      currMap->submap_ = new_JavaUtilHashMap_initWithInt_(1);
    }
    OrgApacheLuceneAnalysisNormalizeCharMap *map = [currMap->submap_ getWithId:JavaLangCharacter_valueOfWithChar_(c)];
    if (map == nil) {
      map = new_OrgApacheLuceneAnalysisNormalizeCharMap_init();
      (void) [((id<JavaUtilMap>) nil_chk(currMap->submap_)) putWithId:JavaLangCharacter_valueOfWithChar_(c) withId:map];
    }
    currMap = map;
  }
  if (currMap->normStr_ != nil) {
    @throw new_JavaLangRuntimeException_initWithNSString_(JreStrcat("$$", @"MappingCharFilter: there is already a mapping for ", singleMatch));
  }
  currMap->normStr_ = replacement;
