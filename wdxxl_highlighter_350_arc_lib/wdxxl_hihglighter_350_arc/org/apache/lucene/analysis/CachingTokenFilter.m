...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/analysis/CachingTokenFilter.java
//
...
- (jboolean)incrementToken {
  if (cache_ == nil) {
    cache_ = new_JavaUtilLinkedList_init();
    OrgApacheLuceneAnalysisCachingTokenFilter_fillCache(self);
    iterator_ = [((id<JavaUtilList>) nil_chk(cache_)) iterator];
  }
...
void OrgApacheLuceneAnalysisCachingTokenFilter_fillCache(OrgApacheLuceneAnalysisCachingTokenFilter *self) {
  ...
  [self->input_ end];
  self->finalState_ = [self captureState];
}
