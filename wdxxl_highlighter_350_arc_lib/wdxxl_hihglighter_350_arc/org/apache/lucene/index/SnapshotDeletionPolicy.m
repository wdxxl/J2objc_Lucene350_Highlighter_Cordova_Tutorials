...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SnapshotDeletionPolicy.java
//
...
- (void)checkSnapshottedWithNSString:(NSString *)id_ {
  if ([self isSnapshottedWithNSString:id_]) {
    @throw new_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$$$", @"Snapshot ID ", id_, @" is already used - must be unique"));
  }
}
...
- (void)registerSnapshotInfoWithNSString:(NSString *)id_
     ...
     withOrgApacheLuceneIndexIndexCommit:(OrgApacheLuceneIndexIndexCommit *)commit {
  (void) [((id<JavaUtilMap>) nil_chk(idToSnapshot_)) putWithId:id_ withId:new_OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo_initWithNSString_withNSString_withOrgApacheLuceneIndexIndexCommit_(id_, segment, commit)];
  id<JavaUtilSet> ids = [((id<JavaUtilMap>) nil_chk(segmentsFileToIDs_)) getWithId:segment];
  if (ids == nil) {
    ids = new_JavaUtilHashSet_init();
    (void) [((id<JavaUtilMap>) nil_chk(segmentsFileToIDs_)) putWithId:segment withId:ids];
  }
  [ids addWithId:id_];
}

- (id<JavaUtilList>)wrapCommitsWithJavaUtilList:(id<JavaUtilList>)commits {
  id<JavaUtilList> wrappedCommits = new_JavaUtilArrayList_initWithInt_([((id<JavaUtilList>) nil_chk(commits)) size]);
  for (OrgApacheLuceneIndexIndexCommit * __strong ic in commits) {
    [wrappedCommits addWithId:new_OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotCommitPoint_initWithOrgApacheLuceneIndexSnapshotDeletionPolicy_withOrgApacheLuceneIndexIndexCommit_(self, ic)];
  }
...
- (OrgApacheLuceneIndexIndexCommit *)getSnapshotWithNSString:(NSString *)id_ {
  @synchronized(self) {
    ...
    if (snapshotInfo == nil) {
      @throw new_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$$", @"No snapshot exists by ID: ", id_));
    }
...
- (id<JavaUtilMap>)getSnapshots {
  @synchronized(self) {
    id<JavaUtilMap> snapshots = new_JavaUtilHashMap_init();
    for (id<JavaUtilMap_Entry> __strong e in nil_chk([((id<JavaUtilMap>) nil_chk(idToSnapshot_)) entrySet])) {
      (void) [snapshots putWithId:[((id<JavaUtilMap_Entry>) nil_chk(e)) getKey] withId:((OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *) nil_chk([e getValue]))->segmentsFileName_];
    }
    return snapshots;
  }
}
...
- (void)onCommitWithJavaUtilList:(id<JavaUtilList>)commits {
  @synchronized(self) {
    ...
    lastCommit_ = [((id<JavaUtilList>) nil_chk(commits)) getWithInt:[commits size] - 1];
  }
}
  ...
  @synchronized(self) {
    ...
    for (OrgApacheLuceneIndexIndexCommit * __strong commit in commits) {
      ...
      if (ids != nil) {
        for (NSString * __strong id_ in ids) {
          ((OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *) nil_chk([((id<JavaUtilMap>) nil_chk(idToSnapshot_)) getWithId:id_]))->commit_ = commit;
        }
      }
    }
    ...
    for (id<JavaUtilMap_Entry> __strong e in nil_chk([((id<JavaUtilMap>) nil_chk(idToSnapshot_)) entrySet])) {
      if (((OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *) nil_chk([((id<JavaUtilMap_Entry>) nil_chk(e)) getValue]))->commit_ == nil) {
        if (idsToRemove == nil) {
          idsToRemove = new_JavaUtilArrayList_init();
        }
    ...
    if (idsToRemove != nil) {
      for (NSString * __strong id_ in idsToRemove) {
        ...
        (void) [((id<JavaUtilMap>) nil_chk(segmentsFileToIDs_)) removeWithId:((OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *) nil_chk(info))->segmentsFileName_];
      }
    }
  }
}
...
- (void)release__WithNSString:(NSString *)id_ {
  @synchronized(self) {
    OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *info = [((id<JavaUtilMap>) nil_chk(idToSnapshot_)) removeWithId:id_];
    if (info == nil) {
      @throw new_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$$", @"Snapshot doesn't exist: ", id_));
    }
    id<JavaUtilSet> ids = [((id<JavaUtilMap>) nil_chk(segmentsFileToIDs_)) getWithId:info->segmentsFileName_];
    if (ids != nil) {
      [ids removeWithId:id_];
      if ([ids size] == 0) {
        (void) [((id<JavaUtilMap>) nil_chk(segmentsFileToIDs_)) removeWithId:info->segmentsFileName_];
      }
    }
  }
}
...
- (OrgApacheLuceneIndexIndexCommit *)snapshotWithNSString:(NSString *)id_ {
  @synchronized(self) {
    if (lastCommit_ == nil) {
      @throw new_JavaLangIllegalStateException_initWithNSString_(@"No index commit to snapshot");
    }
...
void OrgApacheLuceneIndexSnapshotDeletionPolicy_initWithOrgApacheLuceneIndexIndexDeletionPolicy_(OrgApacheLuceneIndexSnapshotDeletionPolicy *self, id<OrgApacheLuceneIndexIndexDeletionPolicy> primary) {
  NSObject_init(self);
  self->idToSnapshot_ = new_JavaUtilHashMap_init();
  self->segmentsFileToIDs_ = new_JavaUtilHashMap_init();
...
void OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo_initWithNSString_withNSString_withOrgApacheLuceneIndexIndexCommit_(OrgApacheLuceneIndexSnapshotDeletionPolicy_SnapshotInfo *self, NSString *id_, NSString *segmentsFileName, OrgApacheLuceneIndexIndexCommit *commit) {
  NSObject_init(self);
  self->id__ = id_;
  self->segmentsFileName_ = segmentsFileName;
  self->commit_ = commit;
}
