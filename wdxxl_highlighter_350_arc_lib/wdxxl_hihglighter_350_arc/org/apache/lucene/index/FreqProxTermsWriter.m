...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/FreqProxTermsWriter.java
//
...
- (OrgApacheLuceneIndexTermsHashConsumerPerThread *)addThreadWithOrgApacheLuceneIndexTermsHashPerThread:(OrgApacheLuceneIndexTermsHashPerThread *)perThread {
  return new_OrgApacheLuceneIndexFreqProxTermsWriterPerThread_initWithOrgApacheLuceneIndexTermsHashPerThread_(perThread);
}
...
withOrgApacheLuceneIndexSegmentWriteState:(OrgApacheLuceneIndexSegmentWriteState *)state {
  ...
  OrgApacheLuceneIndexFormatPostingsFieldsConsumer *consumer = new_OrgApacheLuceneIndexFormatPostingsFieldsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFieldInfos_(state, fieldInfos_);
  @try {
    jint start = 0;
    while (start < numAllFields) {
      ...
      IOSObjectArray *fields = [IOSObjectArray newArrayWithLength:end - start type:OrgApacheLuceneIndexFreqProxTermsWriterPerField_class_()];
      for (jint i = start; i < end; i++) {
        (void) IOSObjectArray_Set(fields, i - start, [allFields getWithInt:i]);
...
withOrgApacheLuceneIndexFormatPostingsFieldsConsumer:(OrgApacheLuceneIndexFormatPostingsFieldsConsumer *)consumer {
  ...
  IOSObjectArray *termStates = [IOSObjectArray newArrayWithLength:numFields type:OrgApacheLuceneIndexFreqProxFieldMergeState_class_()];
  ...
  @try {
    while (numFields > 0) {
      (void) IOSObjectArray_Set(termStates, 0, IOSObjectArray_Get(mergeStates, 0));
      jint numToMerge = 1;
      for (jint i = 1; i < numFields; i++) {
        ...
        if (cmp < 0) {
          (void) IOSObjectArray_Set(termStates, 0, IOSObjectArray_Get(mergeStates, i));
          numToMerge = 1;
        }
        else if (cmp == 0) (void) IOSObjectArray_Set(termStates, numToMerge++, IOSObjectArray_Get(mergeStates, i));
      }
        ...
        while (numToMerge > 0) {
          ...
          if (minState->docID_ < delDocLimit) {
            if (state->deletedDocs_ == nil) {
              state->deletedDocs_ = new_OrgApacheLuceneUtilBitVector_initWithInt_(state->numDocs_);
            }
            ...
            @try {
              ...
              for (jint j = 0; j < termDocFreq; j++) {
                ...
                if ((code & 1) != 0) {
                  ...
                  if (payloadBuffer_ == nil || payloadBuffer_->size_ < payloadLength) payloadBuffer_ = [IOSByteArray newArrayWithLength:payloadLength];
...
void OrgApacheLuceneIndexFreqProxTermsWriter_init(OrgApacheLuceneIndexFreqProxTermsWriter *self) {
  ...
  self->termsUTF8_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF8Result_init();
}
