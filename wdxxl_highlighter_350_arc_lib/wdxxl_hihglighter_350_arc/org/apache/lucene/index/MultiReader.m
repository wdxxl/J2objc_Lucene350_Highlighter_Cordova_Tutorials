...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/MultiReader.java
//
...
- (OrgApacheLuceneIndexIndexReader *)doOpenIfChangedWithBoolean:(jboolean)doClone {
  ...
  IOSObjectArray *newSubReaders = [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(subReaders_))->size_ type:OrgApacheLuceneIndexIndexReader_class_()];
  jboolean success = false;
  @try {
    for (jint i = 0; i < ((IOSObjectArray *) nil_chk(subReaders_))->size_; i++) {
      if (doClone) {
        (void) IOSObjectArray_Set(newSubReaders, i, (OrgApacheLuceneIndexIndexReader *) cast_chk([((OrgApacheLuceneIndexIndexReader *) nil_chk(IOSObjectArray_Get(subReaders_, i))) java_clone], [OrgApacheLuceneIndexIndexReader class]));
        changed = true;
      }
      else {
        ...
        if (newSubReader != nil) {
          (void) IOSObjectArray_Set(newSubReaders, i, newSubReader);
          changed = true;
        }
        else {
          (void) IOSObjectArray_Set(newSubReaders, i, IOSObjectArray_Get(nil_chk(subReaders_), i));
        }
      }
    }
    success = true;
  ...
  if (changed) {
    IOSBooleanArray *newDecrefOnClose = [IOSBooleanArray newArrayWithLength:((IOSObjectArray *) nil_chk(subReaders_))->size_];
    for (jint i = 0; i < ((IOSObjectArray *) nil_chk(subReaders_))->size_; i++) {
    ...
    OrgApacheLuceneIndexMultiReader *mr = new_OrgApacheLuceneIndexMultiReader_initWithOrgApacheLuceneIndexIndexReaderArray_(newSubReaders);
    mr->decrefOnClose_ = newDecrefOnClose;
    return mr;
  }
  else {
    return nil;
...
- (jlong)getVersion {
  @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"MultiReader does not support this method.");
}
...
void OrgApacheLuceneIndexMultiReader_initialize__WithOrgApacheLuceneIndexIndexReaderArray_withBoolean_(OrgApacheLuceneIndexMultiReader *self, IOSObjectArray *subReaders, jboolean closeSubReaders) {
  self->subReaders_ = [((IOSObjectArray *) nil_chk(subReaders)) java_clone];
  self->starts_ = [IOSIntArray newArrayWithLength:subReaders->size_ + 1];
  self->decrefOnClose_ = [IOSBooleanArray newArrayWithLength:subReaders->size_];
