...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/IndexUpgrader.java
//
...
- (void)upgrade {
  if (!OrgApacheLuceneIndexIndexReader_indexExistsWithOrgApacheLuceneStoreDirectory_(dir_)) {
    @throw new_OrgApacheLuceneIndexIndexNotFoundException_initWithNSString_([((OrgApacheLuceneStoreDirectory *) nil_chk(dir_)) description]);
  }
  if (!deletePriorCommits_) {
    ...
    if ([((id<JavaUtilCollection>) nil_chk(commits)) size] > 1) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"This tool was invoked to not delete prior commit points, but the following commits were found: ", commits));
    }
  }
  ...
  (void) [((OrgApacheLuceneIndexIndexWriterConfig *) nil_chk(c)) setMergePolicyWithOrgApacheLuceneIndexMergePolicy:new_OrgApacheLuceneIndexUpgradeIndexMergePolicy_initWithOrgApacheLuceneIndexMergePolicy_([c getMergePolicy])];
  (void) [c setIndexDeletionPolicyWithOrgApacheLuceneIndexIndexDeletionPolicy:new_OrgApacheLuceneIndexKeepOnlyLastCommitDeletionPolicy_init()];
  OrgApacheLuceneIndexIndexWriter *w = new_OrgApacheLuceneIndexIndexWriter_initWithOrgApacheLuceneStoreDirectory_withOrgApacheLuceneIndexIndexWriterConfig_(dir_, c);
  @try {
...
void OrgApacheLuceneIndexIndexUpgrader_mainWithNSStringArray_(IOSObjectArray *args) {
  ...
  [new_OrgApacheLuceneIndexIndexUpgrader_initWithOrgApacheLuceneStoreDirectory_withOrgApacheLuceneUtilVersion_withJavaIoPrintStream_withBoolean_(OrgApacheLuceneStoreFSDirectory_openWithJavaIoFile_(new_JavaIoFile_initWithNSString_(dir)), JreLoadEnum(OrgApacheLuceneUtilVersion, LUCENE_CURRENT), out, deletePriorCommits) upgrade];
}
...
void OrgApacheLuceneIndexIndexUpgrader_initWithOrgApacheLuceneStoreDirectory_withOrgApacheLuceneUtilVersion_(OrgApacheLuceneIndexIndexUpgrader *self, OrgApacheLuceneStoreDirectory *dir, OrgApacheLuceneUtilVersion *matchVersion) {
  OrgApacheLuceneIndexIndexUpgrader_initWithOrgApacheLuceneStoreDirectory_withOrgApacheLuceneIndexIndexWriterConfig_withJavaIoPrintStream_withBoolean_(self, dir, new_OrgApacheLuceneIndexIndexWriterConfig_initWithOrgApacheLuceneUtilVersion_withOrgApacheLuceneAnalysisAnalyzer_(matchVersion, nil), nil, false);
}
