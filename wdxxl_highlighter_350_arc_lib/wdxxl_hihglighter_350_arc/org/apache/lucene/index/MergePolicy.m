...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/MergePolicy.java
//
...
void OrgApacheLuceneIndexMergePolicy_init(OrgApacheLuceneIndexMergePolicy *self) {
  NSObject_init(self);
  self->writer_ = new_OrgApacheLuceneUtilSetOnce_init();
}
...
- (NSString *)segStringWithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)dir {
  ...
  for (jint i = 0; i < numSegments; i++) {
    if (i > 0) (void) [b appendWithChar:' '];
    (void) [b appendWithNSString:[((OrgApacheLuceneIndexSegmentInfo *) nil_chk([segments_ getWithInt:i])) toStringWithOrgApacheLuceneStoreDirectory:dir withInt:0]];
  }
  if (info_ != nil) (void) [((JavaLangStringBuilder *) nil_chk([b appendWithNSString:@" into "])) appendWithNSString:((OrgApacheLuceneIndexSegmentInfo *) nil_chk(info_))->name_];
  if (maxNumSegments_ != -1) (void) [b appendWithNSString:JreStrcat("$IC", @" [maxNumSegments=", maxNumSegments_, ']')];
  if (aborted_) {
    (void) [b appendWithNSString:@" [ABORTED]"];
  }
...
void OrgApacheLuceneIndexMergePolicy_OneMerge_initWithJavaUtilList_(OrgApacheLuceneIndexMergePolicy_OneMerge *self, id<JavaUtilList> segments) {
  ...
  if (0 == [((id<JavaUtilList>) nil_chk(segments)) size]) @throw new_JavaLangRuntimeException_initWithNSString_(@"segments must include at least one segment");
  self->segments_ = new_JavaUtilArrayList_initWithJavaUtilCollection_(segments);
  jint count = 0;
...
void OrgApacheLuceneIndexMergePolicy_MergeSpecification_init(OrgApacheLuceneIndexMergePolicy_MergeSpecification *self) {
  NSObject_init(self);
  self->merges_ = new_JavaUtilArrayList_init();
}
