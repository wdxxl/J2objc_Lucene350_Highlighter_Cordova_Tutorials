...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/DocFieldProcessorPerThread.java
//
...
- (id<JavaUtilCollection>)fields {
  id<JavaUtilCollection> fields = new_JavaUtilHashSet_init();
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(fieldHash_))->size_; i++) {
    ...
    while (perField != nil) {
      if (perField->lastGen_ == -1) {
        if (lastPerField == nil) (void) IOSObjectArray_Set(nil_chk(fieldHash_), i, perField->next_);
        else lastPerField->next_ = perField->next_;
...
- (OrgApacheLuceneIndexDocumentsWriter_DocWriter *)processDocument {
  ...
  for (jint i = 0; i < numDocFields; i++) {
    ...
    if (fp == nil) {
      ...
      fp = new_OrgApacheLuceneIndexDocFieldProcessorPerField_initWithOrgApacheLuceneIndexDocFieldProcessorPerThread_withOrgApacheLuceneIndexFieldInfo_(self, fi);
      fp->next_ = IOSObjectArray_Get(nil_chk(fieldHash_), hashPos);
      (void) IOSObjectArray_Set(fieldHash_, hashPos, fp);
      totalFieldCount_++;
    ...
    if (thisFieldGen != fp->lastGen_) {
      ...
      if (fieldCount_ == ((IOSObjectArray *) nil_chk(fields_))->size_) {
        ...
        IOSObjectArray *newArray = [IOSObjectArray newArrayWithLength:newSize type:OrgApacheLuceneIndexDocFieldProcessorPerField_class_()];
        JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(fields_, 0, newArray, 0, fieldCount_);
        fields_ = newArray;
      }
      (void) IOSObjectArray_Set(fields_, fieldCount_++, fp);
      fp->lastGen_ = thisFieldGen;
    }
    if (fp->fieldCount_ == ((IOSObjectArray *) nil_chk(fp->fields_))->size_) {
      IOSObjectArray *newArray = [IOSObjectArray newArrayWithLength:fp->fields_->size_ * 2 type:OrgApacheLuceneDocumentFieldable_class_()];
      JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(fp->fields_, 0, newArray, 0, fp->fieldCount_);
      fp->fields_ = newArray;
    }
    (void) IOSObjectArray_Set(fp->fields_, fp->fieldCount_++, field);
    if ([field isStored]) {
...
+ (void)initialize {
  if (self == [OrgApacheLuceneIndexDocFieldProcessorPerThread class]) {
    OrgApacheLuceneIndexDocFieldProcessorPerThread_fieldsComp = new_OrgApacheLuceneIndexDocFieldProcessorPerThread_1_init();
...
void OrgApacheLuceneIndexDocFieldProcessorPerThread_initWithOrgApacheLuceneIndexDocumentsWriterThreadState_withOrgApacheLuceneIndexDocFieldProcessor_(OrgApacheLuceneIndexDocFieldProcessorPerThread *self, OrgApacheLuceneIndexDocumentsWriterThreadState *threadState, OrgApacheLuceneIndexDocFieldProcessor *docFieldProcessor) {
  ...
  self->fields_ = [IOSObjectArray newArrayWithLength:1 type:OrgApacheLuceneIndexDocFieldProcessorPerField_class_()];
  self->fieldHash_ = [IOSObjectArray newArrayWithLength:2 type:OrgApacheLuceneIndexDocFieldProcessorPerField_class_()];
  self->hashMask_ = 1;
  self->docFreeList_ = [IOSObjectArray newArrayWithLength:1 type:OrgApacheLuceneIndexDocFieldProcessorPerThread_PerDoc_class_()];
  self->docState_ = ((OrgApacheLuceneIndexDocumentsWriterThreadState *) nil_chk(threadState))->docState_;
  self->docFieldProcessor_ = docFieldProcessor;
  self->fieldInfos_ = ((OrgApacheLuceneIndexDocFieldProcessor *) nil_chk(docFieldProcessor))->fieldInfos_;
  self->consumer_ = [((OrgApacheLuceneIndexDocFieldConsumer *) nil_chk(docFieldProcessor->consumer_)) addThreadWithOrgApacheLuceneIndexDocFieldProcessorPerThread:self];
  self->fieldsWriter_ = [((OrgApacheLuceneIndexStoredFieldsWriter *) nil_chk(docFieldProcessor->fieldsWriter_)) addThreadWithOrgApacheLuceneIndexDocumentsWriter_DocState:self->docState_];
}
...
void OrgApacheLuceneIndexDocFieldProcessorPerThread_rehash(OrgApacheLuceneIndexDocFieldProcessorPerThread *self) {
  ...
  for (jint j = 0; j < ((IOSObjectArray *) nil_chk(self->fieldHash_))->size_; j++) {
    ...
    while (fp0 != nil) {
      ...
      fp0->next_ = IOSObjectArray_Get(newHashArray, hashPos2);
      (void) IOSObjectArray_Set(newHashArray, hashPos2, fp0);
      fp0 = nextFP0;
    }
  }
  self->fieldHash_ = newHashArray;
