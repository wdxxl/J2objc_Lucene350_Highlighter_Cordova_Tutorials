...
- (void)init__WithLong:(jlong)skipPointer
  ...
  *IOSLongArray_GetRef(nil_chk(self->skipPointer_), 0) = skipPointer;
  ...
  for (jint i = 1; i < numberOfSkipLevels_; i++) {
    (void) IOSObjectArray_Set(nil_chk(skipStream_), i, nil);
  }
}
...
void OrgApacheLuceneIndexMultiLevelSkipListReader_initWithOrgApacheLuceneStoreIndexInput_withInt_withInt_(OrgApacheLuceneIndexMultiLevelSkipListReader *self, OrgApacheLuceneStoreIndexInput *skipStream, jint maxSkipLevels, jint skipInterval) {
  ...
  self->skipStream_ = [IOSObjectArray newArrayWithLength:maxSkipLevels type:OrgApacheLuceneStoreIndexInput_class_()];
  self->skipPointer_ = [IOSLongArray newArrayWithLength:maxSkipLevels];
  self->childPointer_ = [IOSLongArray newArrayWithLength:maxSkipLevels];
  self->numSkipped_ = [IOSIntArray newArrayWithLength:maxSkipLevels];
  self->maxNumberOfSkipLevels_ = maxSkipLevels;
  self->skipInterval_ = [IOSIntArray newArrayWithLength:maxSkipLevels];
  (void) IOSObjectArray_Set(self->skipStream_, 0, skipStream);
  ...
  *IOSIntArray_GetRef(self->skipInterval_, 0) = skipInterval;
  ...
  self->skipDoc_ = [IOSIntArray newArrayWithLength:maxSkipLevels];
}
...
void OrgApacheLuceneIndexMultiLevelSkipListReader_loadSkipLevels(OrgApacheLuceneIndexMultiLevelSkipListReader *self) {
  ...
  for (jint i = self->numberOfSkipLevels_ - 1; i > 0; i--) {
    ...
    *IOSLongArray_GetRef(nil_chk(self->skipPointer_), i) = [((OrgApacheLuceneStoreIndexInput *) nil_chk(IOSObjectArray_Get(nil_chk(self->skipStream_), 0))) getFilePointer];
    if (toBuffer > 0) {
      (void) IOSObjectArray_SetAndConsume(nil_chk(self->skipStream_), i, new_OrgApacheLuceneIndexMultiLevelSkipListReader_SkipBuffer_initWithOrgApacheLuceneStoreIndexInput_withInt_(IOSObjectArray_Get(self->skipStream_, 0), (jint) length));
      toBuffer--;
    }
    else {
      (void) IOSObjectArray_Set(nil_chk(self->skipStream_), i, (OrgApacheLuceneStoreIndexInput *) cast_chk([((OrgApacheLuceneStoreIndexInput *) nil_chk(IOSObjectArray_Get(self->skipStream_, 0))) java_clone], [OrgApacheLuceneStoreIndexInput class]));
...
void OrgApacheLuceneIndexMultiLevelSkipListReader_SkipBuffer_initWithOrgApacheLuceneStoreIndexInput_withInt_(OrgApacheLuceneIndexMultiLevelSkipListReader_SkipBuffer *self, OrgApacheLuceneStoreIndexInput *input, jint length) {
  ...
  self->data_ = [IOSByteArray newArrayWithLength:length];
