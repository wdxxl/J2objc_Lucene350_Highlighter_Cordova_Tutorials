...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/FreqProxFieldMergeState.java
//
...
- (jboolean)nextTerm {
  ...
  text_ = IOSObjectArray_Get(nil_chk(((OrgApacheLuceneIndexCharBlockPool *) nil_chk(charPool_))->buffers_), JreRShift32(textStart, OrgApacheLuceneIndexDocumentsWriter_CHAR_BLOCK_SHIFT));
...
void OrgApacheLuceneIndexFreqProxFieldMergeState_initWithOrgApacheLuceneIndexFreqProxTermsWriterPerField_(OrgApacheLuceneIndexFreqProxFieldMergeState *self, OrgApacheLuceneIndexFreqProxTermsWriterPerField *field) {
  NSObject_init(self);
  self->postingUpto_ = -1;
  self->freq_ = new_OrgApacheLuceneIndexByteSliceReader_init();
  self->prox_ = new_OrgApacheLuceneIndexByteSliceReader_init();
  self->field_ = field;
  self->charPool_ = ((OrgApacheLuceneIndexTermsHashPerThread *) nil_chk(((OrgApacheLuceneIndexFreqProxTermsWriterPerThread *) nil_chk(((OrgApacheLuceneIndexFreqProxTermsWriterPerField *) nil_chk(field))->perThread_))->termsHashPerThread_))->charPool_;
  self->numPostings_ = ((OrgApacheLuceneIndexTermsHashPerField *) nil_chk(field->termsHashPerField_))->numPostings_;
  self->termIDs_ = [field->termsHashPerField_ sortPostings];
  self->postings_ = (OrgApacheLuceneIndexFreqProxTermsWriterPerField_FreqProxPostingsArray *) cast_chk(field->termsHashPerField_->postingsArray_, [OrgApacheLuceneIndexFreqProxTermsWriterPerField_FreqProxPostingsArray class]);
}
