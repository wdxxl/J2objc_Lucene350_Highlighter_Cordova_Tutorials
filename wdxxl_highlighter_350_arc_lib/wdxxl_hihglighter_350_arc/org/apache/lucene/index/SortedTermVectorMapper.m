...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SortedTermVectorMapper.java
//
...
withOrgApacheLuceneIndexTermVectorOffsetInfoArray:(IOSObjectArray *)offsets
  ...
  if (entry_ == nil) {
    entry_ = new_OrgApacheLuceneIndexTermVectorEntry_initWithNSString_withNSString_withInt_withOrgApacheLuceneIndexTermVectorOffsetInfoArray_withIntArray_(OrgApacheLuceneIndexSortedTermVectorMapper_ALL, term, frequency, storeOffsets_ == true ? offsets : nil, storePositions_ == true ? positions : nil);
    (void) [((id<JavaUtilMap>) nil_chk(termToTVE_)) putWithId:term withId:entry_];
    ...
    if (storeOffsets_) {
      ...
      if (existingOffsets != nil && offsets != nil && offsets->size_ > 0) {
        IOSObjectArray *newOffsets = [IOSObjectArray newArrayWithLength:existingOffsets->size_ + offsets->size_ type:OrgApacheLuceneIndexTermVectorOffsetInfo_class_()];
    ...
    if (storePositions_) {
      ...
      if (existingPositions != nil && positions != nil && positions->size_ > 0) {
        IOSIntArray *newPositions = [IOSIntArray newArrayWithLength:existingPositions->size_ + positions->size_];
...
void OrgApacheLuceneIndexSortedTermVectorMapper_initWithBoolean_withBoolean_withJavaUtilComparator_(OrgApacheLuceneIndexSortedTermVectorMapper *self, jboolean ignoringPositions, jboolean ignoringOffsets, id<JavaUtilComparator> comparator) {
  ...
  self->termToTVE_ = new_JavaUtilHashMap_init();
  self->currentSet_ = new_JavaUtilTreeSet_initWithJavaUtilComparator_(comparator);
}
