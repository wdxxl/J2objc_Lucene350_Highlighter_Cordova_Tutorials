...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/PersistentSnapshotDeletionPolicy.java
//
...
id<JavaUtilMap> OrgApacheLuceneIndexPersistentSnapshotDeletionPolicy_readSnapshotsInfoWithOrgApacheLuceneStoreDirectory_(OrgApacheLuceneStoreDirectory *dir) {
  ...
  @try {
    ...
    if (numDocs == 1) {
      ...
      if (sid == nil) {
        @throw new_JavaLangIllegalStateException_initWithNSString_(@"directory is not a valid snapshots store!");
      }
      ...
      for (id<OrgApacheLuceneDocumentFieldable> __strong f in nil_chk([doc getFields])) {
        (void) [snapshots putWithId:[((id<OrgApacheLuceneDocumentFieldable>) nil_chk(f)) name] withId:[f stringValue]];
      }
    }
    else if (numDocs != 0) {
      @throw new_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$I", @"should be at most 1 document in the snapshots directory: ", numDocs));
    }
  }
  @finally {
    [r close];
