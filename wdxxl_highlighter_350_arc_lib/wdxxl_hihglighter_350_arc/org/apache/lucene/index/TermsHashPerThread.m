...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/TermsHashPerThread.java
//
...
- (OrgApacheLuceneIndexInvertedDocConsumerPerField *)addFieldWithOrgApacheLuceneIndexDocInverterPerField:(OrgApacheLuceneIndexDocInverterPerField *)docInverterPerField
  ...
  return new_OrgApacheLuceneIndexTermsHashPerField_initWithOrgApacheLuceneIndexDocInverterPerField_withOrgApacheLuceneIndexTermsHashPerThread_withOrgApacheLuceneIndexTermsHashPerThread_withOrgApacheLuceneIndexFieldInfo_(docInverterPerField, self, nextPerThread_, fieldInfo);
}
...
void OrgApacheLuceneIndexTermsHashPerThread_initWithOrgApacheLuceneIndexDocInverterPerThread_withOrgApacheLuceneIndexTermsHash_withOrgApacheLuceneIndexTermsHash_withOrgApacheLuceneIndexTermsHashPerThread_(OrgApacheLuceneIndexTermsHashPerThread *self, OrgApacheLuceneIndexDocInverterPerThread *docInverterPerThread, OrgApacheLuceneIndexTermsHash *termsHash, OrgApacheLuceneIndexTermsHash *nextTermsHash, OrgApacheLuceneIndexTermsHashPerThread *primaryPerThread) {
  ...
  self->docState_ = ((OrgApacheLuceneIndexDocInverterPerThread *) nil_chk(docInverterPerThread))->docState_;
  self->termsHash_ = termsHash;
  self->consumer_ = [((OrgApacheLuceneIndexTermsHashConsumer *) nil_chk(((OrgApacheLuceneIndexTermsHash *) nil_chk(termsHash))->consumer_)) addThreadWithOrgApacheLuceneIndexTermsHashPerThread:self];
  if (nextTermsHash != nil) {
    self->charPool_ = new_OrgApacheLuceneIndexCharBlockPool_initWithOrgApacheLuceneIndexDocumentsWriter_(termsHash->docWriter_);
    self->primary_ = true;
  }
  else {
    self->charPool_ = ((OrgApacheLuceneIndexTermsHashPerThread *) nil_chk(primaryPerThread))->charPool_;
    self->primary_ = false;
  }
  self->intPool_ = new_OrgApacheLuceneIndexIntBlockPool_initWithOrgApacheLuceneIndexDocumentsWriter_(termsHash->docWriter_);
  self->bytePool_ = new_OrgApacheLuceneIndexByteBlockPool_initWithOrgApacheLuceneIndexByteBlockPool_Allocator_(((OrgApacheLuceneIndexDocumentsWriter *) nil_chk(termsHash->docWriter_))->byteBlockAllocator_);
  if (nextTermsHash != nil) self->nextPerThread_ = [nextTermsHash addThreadWithOrgApacheLuceneIndexDocInverterPerThread:docInverterPerThread withOrgApacheLuceneIndexTermsHashPerThread:self];
  else self->nextPerThread_ = nil;
}
