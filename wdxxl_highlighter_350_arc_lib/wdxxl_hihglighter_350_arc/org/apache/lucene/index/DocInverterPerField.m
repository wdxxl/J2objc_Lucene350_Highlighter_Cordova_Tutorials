...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/DocInverterPerField.java
//
  ...
  for (jint i = 0; i < count; i++) {
    ...
    if ([((id<OrgApacheLuceneDocumentFieldable>) nil_chk(field)) isIndexed] && doInvert) {
      ...
      if (![field isTokenized]) {
        ...
        fieldState_->attributeSource_ = perThread_->singleToken_;
        ...
        if (streamValue != nil) stream = streamValue;
          ...
          if (readerValue != nil) reader = readerValue;
            ...
            NSString *stringValue = [field stringValue];
            if (stringValue == nil) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"field must have either TokenStream, String or Reader value");
        ...
        @try {
          ...
          fieldState_->attributeSource_ = stream;
    ...
    (void) IOSObjectArray_Set(fields, i, nil);
  }
  [consumer_ finish];
...
void OrgApacheLuceneIndexDocInverterPerField_initWithOrgApacheLuceneIndexDocInverterPerThread_withOrgApacheLuceneIndexFieldInfo_(OrgApacheLuceneIndexDocInverterPerField *self, OrgApacheLuceneIndexDocInverterPerThread *perThread, OrgApacheLuceneIndexFieldInfo *fieldInfo) {
  ...
  self->docState_ = ((OrgApacheLuceneIndexDocInverterPerThread *) nil_chk(perThread))->docState_;
  self->fieldState_ = perThread->fieldState_;
  self->consumer_ = [((OrgApacheLuceneIndexInvertedDocConsumerPerThread *) nil_chk(perThread->consumer_)) addFieldWithOrgApacheLuceneIndexDocInverterPerField:self withOrgApacheLuceneIndexFieldInfo:fieldInfo];
  self->endConsumer_ = [((OrgApacheLuceneIndexInvertedDocEndConsumerPerThread *) nil_chk(perThread->endConsumer_)) addFieldWithOrgApacheLuceneIndexDocInverterPerField:self withOrgApacheLuceneIndexFieldInfo:fieldInfo];
}
