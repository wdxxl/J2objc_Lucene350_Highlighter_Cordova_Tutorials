...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/CompoundFileReader.java
//
...
- (void)close {
  @synchronized(self) {
    if (stream_ == nil) @throw new_JavaIoIOException_initWithNSString_(@"Already closed");
...
- (OrgApacheLuceneStoreIndexInput *)openInputWithNSString:(NSString *)id_
                                                  withInt:(jint)readBufferSize {
  @synchronized(self) {
    if (stream_ == nil) @throw new_JavaIoIOException_initWithNSString_(@"Stream closed");
    ...
    if (entry_ == nil) {
      @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$$$$$@C", @"No sub-file with id ", id_, @" found (fileName=", fileName_, @" files: ", [((JavaUtilHashMap *) nil_chk(entries_)) keySet], ')'));
    }
    return new_OrgApacheLuceneIndexCompoundFileReader_CSIndexInput_initWithOrgApacheLuceneStoreIndexInput_withLong_withLong_withInt_(stream_, entry_->offset_, entry_->length_, readBufferSize);
  }
}

- (IOSObjectArray *)listAll {
  IOSObjectArray *res = [((id<JavaUtilSet>) nil_chk([((JavaUtilHashMap *) nil_chk(entries_)) keySet])) toArrayWithNSObjectArray:[IOSObjectArray newArrayWithLength:[((JavaUtilHashMap *) nil_chk(entries_)) size] type:NSString_class_()]];
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(res))->size_; i++) {
    (void) IOSObjectArray_Set(res, i, JreStrcat("$$", seg, IOSObjectArray_Get(res, i)));
  }
  return res;
}
...
- (jlong)fileLengthWithNSString:(NSString *)name {
  ...
  if (e == nil) @throw new_JavaIoFileNotFoundException_initWithNSString_(name);
  return e->length_;
}
...
void OrgApacheLuceneIndexCompoundFileReader_initWithOrgApacheLuceneStoreDirectory_withNSString_withInt_(OrgApacheLuceneIndexCompoundFileReader *self, OrgApacheLuceneStoreDirectory *dir, NSString *name, jint readBufferSize) {
  ...
  @try {
    self->stream_ = [((OrgApacheLuceneStoreDirectory *) nil_chk(dir)) openInputWithNSString:name withInt:readBufferSize];
    ...
    if (firstInt < OrgApacheLuceneIndexCompoundFileWriter_FORMAT_PRE_VERSION) {
      if (firstInt < OrgApacheLuceneIndexCompoundFileWriter_FORMAT_CURRENT) {
        @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$I$I$@C", @"Incompatible format version: ", firstInt, @" expected ", OrgApacheLuceneIndexCompoundFileWriter_FORMAT_CURRENT, @" (resource: ", self->stream_, ')'));
      }
    ...
    for (jint i = 0; i < count; i++) {
      ...
      entry_ = new_OrgApacheLuceneIndexCompoundFileReader_FileEntry_init();
      entry_->offset_ = offset;
      (void) [((JavaUtilHashMap *) nil_chk(self->entries_)) putWithId:id_ withId:entry_];
    }
    if (entry_ != nil) {
...
- (id)java_clone {
  ...
  ((OrgApacheLuceneIndexCompoundFileReader_CSIndexInput *) nil_chk(clone))->base_ = (OrgApacheLuceneStoreIndexInput *) cast_chk([((OrgApacheLuceneStoreIndexInput *) nil_chk(base_)) java_clone], [OrgApacheLuceneStoreIndexInput class]);
...
- (void)readInternalWithByteArray:(IOSByteArray *)b
  ...
  if (start + len > length_) @throw new_JavaIoEOFException_initWithNSString_(JreStrcat("$@C", @"read past EOF (resource: ", base_, ')'));
...
- (void)copyBytesWithOrgApacheLuceneStoreIndexOutput:(OrgApacheLuceneStoreIndexOutput *)outArg
  ...
  if (numBytes > 0) {
    ...
    if (start + numBytes > length_) {
      @throw new_JavaIoEOFException_initWithNSString_(JreStrcat("$@C", @"read past EOF (resource: ", base_, ')'));
    }
    [((OrgApacheLuceneStoreIndexInput *) nil_chk(base_)) seekWithLong:fileOffset_ + start];
...
void OrgApacheLuceneIndexCompoundFileReader_CSIndexInput_initWithOrgApacheLuceneStoreIndexInput_withLong_withLong_withInt_(OrgApacheLuceneIndexCompoundFileReader_CSIndexInput *self, OrgApacheLuceneStoreIndexInput *base, jlong fileOffset, jlong length, jint readBufferSize) {
  ...
  self->base_ = (OrgApacheLuceneStoreIndexInput *) cast_chk([((OrgApacheLuceneStoreIndexInput *) nil_chk(base)) java_clone], [OrgApacheLuceneStoreIndexInput class]);
