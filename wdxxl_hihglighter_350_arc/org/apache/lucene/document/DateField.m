...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/document/DateField.java
//
...
NSString *OrgApacheLuceneDocumentDateField_MAX_DATE_STRING() {
  OrgApacheLuceneDocumentDateField_initialize();
  IOSCharArray *buffer = [IOSCharArray newArrayWithLength:OrgApacheLuceneDocumentDateField_DATE_LEN];
...
NSString *OrgApacheLuceneDocumentDateField_timeToStringWithLong_(jlong time) {
  OrgApacheLuceneDocumentDateField_initialize();
  if (time < 0) @throw new_JavaLangRuntimeException_initWithNSString_(JreStrcat("$J$", @"time '", time, @"' is too early, must be >= 0"));
  NSString *s = JavaLangLong_toStringWithLong_withInt_(time, JavaLangCharacter_MAX_RADIX);
  if ([((NSString *) nil_chk(s)) java_length] > OrgApacheLuceneDocumentDateField_DATE_LEN) @throw new_JavaLangRuntimeException_initWithNSString_(JreStrcat("$J$I", @"time '", time, @"' is too late, length of string representation must be <= ", OrgApacheLuceneDocumentDateField_DATE_LEN));
  if ([s java_length] < OrgApacheLuceneDocumentDateField_DATE_LEN) {
    ...
    while ([sb java_length] < OrgApacheLuceneDocumentDateField_DATE_LEN) (void) [sb insertWithInt:0 withInt:0];
    s = [sb description];
  }
  return s;
}
...
JavaUtilDate *OrgApacheLuceneDocumentDateField_stringToDateWithNSString_(NSString *s) {
  OrgApacheLuceneDocumentDateField_initialize();
  return new_JavaUtilDate_initWithLong_(OrgApacheLuceneDocumentDateField_stringToTimeWithNSString_(s));
}
