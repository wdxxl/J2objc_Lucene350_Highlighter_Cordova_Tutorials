...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/CharsRef.java
//
...
- (id)java_clone {
  return new_OrgApacheLuceneUtilCharsRef_initWithOrgApacheLuceneUtilCharsRef_(self);
}
...
- (void)copy__WithOrgApacheLuceneUtilCharsRef:(OrgApacheLuceneUtilCharsRef *)other {
  if (chars_ == nil) {
    chars_ = [IOSCharArray newArrayWithLength:((OrgApacheLuceneUtilCharsRef *) nil_chk(other))->length_];
  }
  else {
    chars_ = OrgApacheLuceneUtilArrayUtil_growWithCharArray_withInt_(chars_, ((OrgApacheLuceneUtilCharsRef *) nil_chk(other))->length_);
  }
...
- (void)growWithInt:(jint)newLength {
  if (((IOSCharArray *) nil_chk(chars_))->size_ < newLength) {
    chars_ = OrgApacheLuceneUtilArrayUtil_growWithCharArray_withInt_(chars_, newLength);
  }
}
...
- (id<JavaLangCharSequence>)subSequenceFrom:(jint)start
                                         to:(jint)end {
  return new_OrgApacheLuceneUtilCharsRef_initWithCharArray_withInt_withInt_(chars_, offset_ + start, offset_ + end - 1);
}
...
+ (void)initialize {
  if (self == [OrgApacheLuceneUtilCharsRef class]) {
    OrgApacheLuceneUtilCharsRef_EMPTY_ARRAY = [IOSCharArray newArrayWithLength:0];
...
void OrgApacheLuceneUtilCharsRef_initWithNSString_(OrgApacheLuceneUtilCharsRef *self, NSString *string) {
  NSObject_init(self);
  self->chars_ = [((NSString *) nil_chk(string)) java_toCharArray];
  self->offset_ = 0;
