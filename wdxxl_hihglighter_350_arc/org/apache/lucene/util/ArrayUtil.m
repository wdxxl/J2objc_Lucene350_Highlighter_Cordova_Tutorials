...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/ArrayUtil.java
//
...
jint OrgApacheLuceneUtilArrayUtil_parseIntWithCharArray_withInt_withInt_withInt_(IOSCharArray *chars, jint offset, jint len, jint radix) {
  ...
  jint i = 0;
  if (len == 0) {
    @throw new_JavaLangNumberFormatException_initWithNSString_(@"chars length is 0");
  }
  jboolean negative = IOSCharArray_Get(chars, offset + i) == '-';
  if (negative && ++i == len) {
    @throw new_JavaLangNumberFormatException_initWithNSString_(@"can't convert to an int");
  }
  if (negative == true) {
    offset++;
    len--;
...
jint OrgApacheLuceneUtilArrayUtil_parseWithCharArray_withInt_withInt_withInt_withBoolean_(IOSCharArray *chars, jint offset, jint len, jint radix, jboolean negative) {
  ...
  for (jint i = 0; i < len; i++) {
    ...
    if (digit == -1) {
      @throw new_JavaLangNumberFormatException_initWithNSString_(@"Unable to parse");
    }
    if (max > result) {
      @throw new_JavaLangNumberFormatException_initWithNSString_(@"Unable to parse");
    }
...
jint OrgApacheLuceneUtilArrayUtil_oversizeWithInt_withInt_(jint minTargetSize, jint bytesPerElement) {
  OrgApacheLuceneUtilArrayUtil_initialize();
  if (minTargetSize < 0) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I", @"invalid array size ", minTargetSize));
  }
  if (minTargetSize == 0) {
    return 0;
  }
...
IOSIntArray *OrgApacheLuceneUtilArrayUtil_toIntArrayWithJavaUtilCollection_(id<JavaUtilCollection> ints) {
  OrgApacheLuceneUtilArrayUtil_initialize();
  IOSIntArray *result = [IOSIntArray newArrayWithLength:[((id<JavaUtilCollection>) nil_chk(ints)) size]];
  jint upto = 0;
...
OrgApacheLuceneUtilSorterTemplate *OrgApacheLuceneUtilArrayUtil_getSorterWithNSObjectArray_withJavaUtilComparator_(IOSObjectArray *a, id<JavaUtilComparator> comp) {
  OrgApacheLuceneUtilArrayUtil_initialize();
  return new_OrgApacheLuceneUtilArrayUtil_1_initWithNSObjectArray_withJavaUtilComparator_(a, comp);
}

OrgApacheLuceneUtilSorterTemplate *OrgApacheLuceneUtilArrayUtil_getSorterWithJavaLangComparableArray_(IOSObjectArray *a) {
  OrgApacheLuceneUtilArrayUtil_initialize();
  return new_OrgApacheLuceneUtilArrayUtil_2_initWithJavaLangComparableArray_(a);
}
...
- (void)swapWithInt:(jint)i
  ...
  (void) IOSObjectArray_Set(val$a_, i, IOSObjectArray_Get(val$a_, j));
  (void) IOSObjectArray_Set(val$a_, j, o);
}
...
- (void)setPivotWithInt:(jint)i {
  pivot_ = IOSObjectArray_Get(nil_chk(val$a_), i);
}
