...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/RecyclingByteBlockAllocator.java
//
...
- (IOSByteArray *)getByteBlock {
  @synchronized(self) {
    if (freeBlocks_ == 0) {
      ...
      return [IOSByteArray newArrayWithLength:blockSize_];
    }
    IOSByteArray *b = IOSObjectArray_Get(nil_chk(freeByteBlocks_), --freeBlocks_);
    (void) IOSObjectArray_Set(freeByteBlocks_, freeBlocks_, nil);
    return b;
  }
}
...
- (void)recycleByteBlocksWithByteArray2:(IOSObjectArray *)blocks
  ...
  @synchronized(self) {
    ...
    if (size >= ((IOSObjectArray *) nil_chk(freeByteBlocks_))->size_) {
      IOSObjectArray *newBlocks = [IOSObjectArray newArrayWithLength:OrgApacheLuceneUtilArrayUtil_oversizeWithInt_withInt_(size, JreLoadStatic(OrgApacheLuceneUtilRamUsageEstimator, NUM_BYTES_OBJECT_REF)) type:IOSClass_byteArray(1)];
      JavaLangSystem_arraycopyWithId_withInt_withId_withInt_withInt_(freeByteBlocks_, 0, newBlocks, 0, freeBlocks_);
      freeByteBlocks_ = newBlocks;
    }
    jint stop = start + numBlocks;
    for (jint i = start; i < stop; i++) {
      (void) IOSObjectArray_Set(freeByteBlocks_, freeBlocks_++, IOSObjectArray_Get(nil_chk(blocks), i));
...
void OrgApacheLuceneUtilRecyclingByteBlockAllocator_initWithInt_withInt_withJavaUtilConcurrentAtomicAtomicLong_(OrgApacheLuceneUtilRecyclingByteBlockAllocator *self, jint blockSize, jint maxBufferedBlocks, JavaUtilConcurrentAtomicAtomicLong *bytesUsed) {
  ...
  self->freeBlocks_ = 0;
  self->freeByteBlocks_ = [IOSObjectArray newArrayWithLength:JavaLangMath_minWithInt_withInt_(10, maxBufferedBlocks) type:IOSClass_byteArray(1)];
...
void OrgApacheLuceneUtilRecyclingByteBlockAllocator_initWithInt_withInt_(OrgApacheLuceneUtilRecyclingByteBlockAllocator *self, jint blockSize, jint maxBufferedBlocks) {
  OrgApacheLuceneUtilRecyclingByteBlockAllocator_initWithInt_withInt_withJavaUtilConcurrentAtomicAtomicLong_(self, blockSize, maxBufferedBlocks, new_JavaUtilConcurrentAtomicAtomicLong_init());
}
...
void OrgApacheLuceneUtilRecyclingByteBlockAllocator_init(OrgApacheLuceneUtilRecyclingByteBlockAllocator *self) {
  OrgApacheLuceneUtilRecyclingByteBlockAllocator_initWithInt_withInt_withJavaUtilConcurrentAtomicAtomicLong_(self, OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SIZE, 64, new_JavaUtilConcurrentAtomicAtomicLong_init());
}
