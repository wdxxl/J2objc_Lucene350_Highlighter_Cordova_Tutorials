...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/util/BytesRefHash.java
//
...
- (IOSIntArray *)sortWithJavaUtilComparator:(id<JavaUtilComparator>)comp {
  ...
  [new_OrgApacheLuceneUtilBytesRefHash_1_initWithOrgApacheLuceneUtilBytesRefHash_withIntArray_withJavaUtilComparator_(self, compact, comp) quickSortWithInt:0 withInt:count_ - 1];
  return compact;
}
...
- (void)clearWithBoolean:(jboolean)resetPool {
  ...
  bytesStart_ = [((OrgApacheLuceneUtilBytesRefHash_BytesStartArray *) nil_chk(bytesStartArray_)) clear];
  ...
  if (e == -1) {
    ...
    if (len2 + ((OrgApacheLuceneUtilByteBlockPool *) nil_chk(pool_))->byteUpto_ > OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SIZE) {
      if (len2 > OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SIZE) {
        @throw new_OrgApacheLuceneUtilBytesRefHash_MaxBytesLengthExceededException_initWithNSString_(JreStrcat("$I$I", @"bytes can be at most ", (OrgApacheLuceneUtilByteBlockPool_BYTE_BLOCK_SIZE - 2), @" in length; got ", bytes->length_));
      }
      [pool_ nextBuffer];
    }
...
- (void)reinit {
  if (bytesStart_ == nil) {
    bytesStart_ = [((OrgApacheLuceneUtilBytesRefHash_BytesStartArray *) nil_chk(bytesStartArray_)) init__];
  }
  if (ords_ == nil) {
    ords_ = [IOSIntArray newArrayWithLength:hashSize_];
    [((JavaUtilConcurrentAtomicAtomicLong *) nil_chk(bytesUsed_)) addAndGetWithLong:OrgApacheLuceneUtilRamUsageEstimator_NUM_BYTES_INT * hashSize_];
  }
}
...
void OrgApacheLuceneUtilBytesRefHash_init(OrgApacheLuceneUtilBytesRefHash *self) {
  OrgApacheLuceneUtilBytesRefHash_initWithOrgApacheLuceneUtilByteBlockPool_(self, new_OrgApacheLuceneUtilByteBlockPool_initWithOrgApacheLuceneUtilByteBlockPool_Allocator_(new_OrgApacheLuceneUtilByteBlockPool_DirectAllocator_init()));
}
...
void OrgApacheLuceneUtilBytesRefHash_initWithOrgApacheLuceneUtilByteBlockPool_withInt_withOrgApacheLuceneUtilBytesRefHash_BytesStartArray_(OrgApacheLuceneUtilBytesRefHash *self, OrgApacheLuceneUtilByteBlockPool *pool, jint capacity, OrgApacheLuceneUtilBytesRefHash_BytesStartArray *bytesStartArray) {
  NSObject_init(self);
  self->scratch1_ = new_OrgApacheLuceneUtilBytesRef_init();
  self->lastCount_ = -1;
  self->hashSize_ = capacity;
  self->hashHalfSize_ = JreRShift32(self->hashSize_, 1);
  self->hashMask_ = self->hashSize_ - 1;
  self->pool_ = pool;
  self->ords_ = [IOSIntArray newArrayWithLength:self->hashSize_];
  JavaUtilArrays_fillWithIntArray_withInt_(self->ords_, -1);
  self->bytesStartArray_ = bytesStartArray;
  self->bytesStart_ = [((OrgApacheLuceneUtilBytesRefHash_BytesStartArray *) nil_chk(bytesStartArray)) init__];
  self->bytesUsed_ = [bytesStartArray bytesUsed] == nil ? new_JavaUtilConcurrentAtomicAtomicLong_initWithLong_(0) : [bytesStartArray bytesUsed];
...
void OrgApacheLuceneUtilBytesRefHash_1_initWithOrgApacheLuceneUtilBytesRefHash_withIntArray_withJavaUtilComparator_(OrgApacheLuceneUtilBytesRefHash_1 *self, OrgApacheLuceneUtilBytesRefHash *outer$, IOSIntArray *capture$0, id<JavaUtilComparator> capture$1) {
  self->this$0_ = outer$;
  self->val$compact_ = capture$0;
  self->val$comp_ = capture$1;
  OrgApacheLuceneUtilSorterTemplate_init(self);
  self->pivot_ = new_OrgApacheLuceneUtilBytesRef_init();
  self->scratch1_ = new_OrgApacheLuceneUtilBytesRef_init();
  self->scratch2_ = new_OrgApacheLuceneUtilBytesRef_init();
}
...
- (IOSIntArray *)init__ {
  bytesStart_ = [IOSIntArray newArrayWithLength:OrgApacheLuceneUtilArrayUtil_oversizeWithInt_withInt_(initSize_, OrgApacheLuceneUtilRamUsageEstimator_NUM_BYTES_INT)];
  [((JavaUtilConcurrentAtomicAtomicLong *) nil_chk(bytesUsed_)) addAndGetWithLong:(bytesStart_->size_) * OrgApacheLuceneUtilRamUsageEstimator_NUM_BYTES_INT];
  return bytesStart_;
}
...
- (IOSIntArray *)init__ {
  return bytesStart_ = [IOSIntArray newArrayWithLength:OrgApacheLuceneUtilArrayUtil_oversizeWithInt_withInt_(initSize_, OrgApacheLuceneUtilRamUsageEstimator_NUM_BYTES_INT)];
}
