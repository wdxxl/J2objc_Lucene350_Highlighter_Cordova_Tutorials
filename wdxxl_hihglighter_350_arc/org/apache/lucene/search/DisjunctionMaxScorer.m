...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/DisjunctionMaxScorer.java
//
...
void OrgApacheLuceneSearchDisjunctionMaxScorer_heapAdjustWithInt_(OrgApacheLuceneSearchDisjunctionMaxScorer *self, jint root) {
  ...
  while (i <= (JreRShift32(self->numScorers_, 1)) - 1) {
    ...
    if (ldoc < doc) {
      if (rdoc < ldoc) {
        (void) IOSObjectArray_Set(self->subScorers_, i, rscorer);
        (void) IOSObjectArray_Set(self->subScorers_, rchild, scorer);
        i = rchild;
      }
      else {
        (void) IOSObjectArray_Set(self->subScorers_, i, lscorer);
        (void) IOSObjectArray_Set(self->subScorers_, lchild, scorer);
        i = lchild;
      }
    }
    else if (rdoc < doc) {
...
void OrgApacheLuceneSearchDisjunctionMaxScorer_heapRemoveRoot(OrgApacheLuceneSearchDisjunctionMaxScorer *self) {
  if (self->numScorers_ == 1) {
    (void) IOSObjectArray_Set(nil_chk(self->subScorers_), 0, nil);
    self->numScorers_ = 0;
  }
  else {
    (void) IOSObjectArray_Set(nil_chk(self->subScorers_), 0, IOSObjectArray_Get(self->subScorers_, self->numScorers_ - 1));
    (void) IOSObjectArray_Set(self->subScorers_, self->numScorers_ - 1, nil);
    --self->numScorers_;
    OrgApacheLuceneSearchDisjunctionMaxScorer_heapAdjustWithInt_(self, 0);
  }
}
