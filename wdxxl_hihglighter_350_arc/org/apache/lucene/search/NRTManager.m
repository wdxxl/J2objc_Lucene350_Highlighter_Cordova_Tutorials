...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/NRTManager.java
//
...
- (OrgApacheLuceneSearchSearcherManager *)waitForGenerationWithLong:(jlong)targetGen
  ...
  @try {
    ...
    if (targetGen > curGen) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$J$JC", @"targetGen=", targetGen, @" was never returned by this NRTManager instance (current gen=", curGen, ')'));
    }
...
- (void)close {
  ...
  @try {
    @try {
      OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ withDeletes_, withoutDeletes_ } count:2 type:JavaIoCloseable_class_()]);
    }
    @finally {
      [((id<JavaUtilConcurrentLocksCondition>) nil_chk(newGeneration_)) signalAll];
    }
...
void OrgApacheLuceneSearchNRTManager_initWithOrgApacheLuceneIndexIndexWriter_withJavaUtilConcurrentExecutorService_withOrgApacheLuceneSearchSearcherWarmer_withBoolean_(OrgApacheLuceneSearchNRTManager *self, OrgApacheLuceneIndexIndexWriter *writer, id<JavaUtilConcurrentExecutorService> es, id<OrgApacheLuceneSearchSearcherWarmer> warmer, jboolean alwaysApplyDeletes) {
  NSObject_init(self);
  self->waitingListeners_ = new_JavaUtilConcurrentCopyOnWriteArrayList_init();
  self->reopenLock_ = new_JavaUtilConcurrentLocksReentrantLock_init();
  self->newGeneration_ = [self->reopenLock_ newCondition];
  self->writer_ = writer;
  if (alwaysApplyDeletes) {
    self->withoutDeletes_ = self->withDeletes_ = new_OrgApacheLuceneSearchNRTManager_SearcherManagerRef_initWithBoolean_withLong_withOrgApacheLuceneSearchSearcherManager_(true, 0, new_OrgApacheLuceneSearchSearcherManager_initWithOrgApacheLuceneIndexIndexWriter_withBoolean_withOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(writer, true, warmer, es));
  }
  else {
    self->withDeletes_ = new_OrgApacheLuceneSearchNRTManager_SearcherManagerRef_initWithBoolean_withLong_withOrgApacheLuceneSearchSearcherManager_(true, 0, new_OrgApacheLuceneSearchSearcherManager_initWithOrgApacheLuceneIndexIndexWriter_withBoolean_withOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(writer, true, warmer, es));
    self->withoutDeletes_ = new_OrgApacheLuceneSearchNRTManager_SearcherManagerRef_initWithBoolean_withLong_withOrgApacheLuceneSearchSearcherManager_(false, 0, new_OrgApacheLuceneSearchSearcherManager_initWithOrgApacheLuceneIndexIndexWriter_withBoolean_withOrgApacheLuceneSearchSearcherWarmer_withJavaUtilConcurrentExecutorService_(writer, false, warmer, es));
  }
  self->indexingGen_ = new_JavaUtilConcurrentAtomicAtomicLong_initWithLong_(1);
}
