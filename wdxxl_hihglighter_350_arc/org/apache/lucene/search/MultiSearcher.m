...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/MultiSearcher.java
//
...
- (OrgApacheLuceneSearchTopDocs *)searchWithOrgApacheLuceneSearchWeight:(OrgApacheLuceneSearchWeight *)weight
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(searchables_))->size_; i++) {
    OrgApacheLuceneSearchTopDocs *docs = [new_OrgApacheLuceneSearchMultiSearcher_MultiSearcherCallableNoSort_initWithJavaUtilConcurrentLocksLock_withOrgApacheLuceneSearchSearchable_withOrgApacheLuceneSearchWeight_withOrgApacheLuceneSearchFilter_withInt_withOrgApacheLuceneSearchHitQueue_withInt_withIntArray_(JreLoadStatic(OrgApacheLuceneUtilDummyConcurrentLock, INSTANCE), IOSObjectArray_Get(searchables_, i), weight, filter, nDocs, hq, i, starts_) call];
...
- (OrgApacheLuceneSearchTopFieldDocs *)searchWithOrgApacheLuceneSearchWeight:(OrgApacheLuceneSearchWeight *)weight
  ...
  OrgApacheLuceneSearchFieldDocSortedHitQueue *hq = new_OrgApacheLuceneSearchFieldDocSortedHitQueue_initWithInt_(n);
  jint totalHits = 0;
  jfloat maxScore = JavaLangFloat_NEGATIVE_INFINITY;
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(searchables_))->size_; i++) {
    OrgApacheLuceneSearchTopFieldDocs *docs = [new_OrgApacheLuceneSearchMultiSearcher_MultiSearcherCallableWithSort_initWithJavaUtilConcurrentLocksLock_withOrgApacheLuceneSearchSearchable_withOrgApacheLuceneSearchWeight_withOrgApacheLuceneSearchFilter_withInt_withOrgApacheLuceneSearchFieldDocSortedHitQueue_withOrgApacheLuceneSearchSort_withInt_withIntArray_(JreLoadStatic(OrgApacheLuceneUtilDummyConcurrentLock, INSTANCE), IOSObjectArray_Get(searchables_, i), weight, filter, n, hq, sort, i, starts_) call];
...
- (void)searchWithOrgApacheLuceneSearchWeight:(OrgApacheLuceneSearchWeight *)weight
  ...
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(searchables_))->size_; i++) {
    ...
    OrgApacheLuceneSearchCollector *hc = new_OrgApacheLuceneSearchMultiSearcher_1_initWithOrgApacheLuceneSearchCollector_withInt_(collector, start);
...
- (OrgApacheLuceneSearchQuery *)rewriteWithOrgApacheLuceneSearchQuery:(OrgApacheLuceneSearchQuery *)original {
  IOSObjectArray *queries = [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(searchables_))->size_ type:OrgApacheLuceneSearchQuery_class_()];
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(searchables_))->size_; i++) {
    (void) IOSObjectArray_Set(queries, i, [((id<OrgApacheLuceneSearchSearchable>) nil_chk(IOSObjectArray_Get(searchables_, i))) rewriteWithOrgApacheLuceneSearchQuery:original]);
  }
...
- (OrgApacheLuceneSearchWeight *)createNormalizedWeightWithOrgApacheLuceneSearchQuery:(OrgApacheLuceneSearchQuery *)original {
  ...
  id<JavaUtilSet> terms = new_JavaUtilHashSet_init();
  ...
  OrgApacheLuceneSearchMultiSearcher_CachedDfSource *cacheSim = new_OrgApacheLuceneSearchMultiSearcher_CachedDfSource_initWithJavaUtilMap_withInt_withOrgApacheLuceneSearchSimilarity_(dfMap, numDocs, [self getSimilarity]);
...
- (id<JavaUtilMap>)createDocFrequencyMapWithJavaUtilSet:(id<JavaUtilSet>)terms {
  ...
  IOSIntArray *aggregatedDfs = [IOSIntArray newArrayWithLength:((IOSObjectArray *) nil_chk(allTermsArray))->size_];
  {
  ...
  for (jint i = 0; i < allTermsArray->size_; i++) {
    (void) [dfMap putWithId:IOSObjectArray_Get(allTermsArray, i) withId:JavaLangInteger_valueOfWithInt_(IOSIntArray_Get(aggregatedDfs, i))];
  }
  return dfMap;
}
...
void OrgApacheLuceneSearchMultiSearcher_initWithOrgApacheLuceneSearchSearchableArray_(OrgApacheLuceneSearchMultiSearcher *self, IOSObjectArray *searchables) {
  ...
  self->maxDoc_ = 0;
  self->searchables_ = searchables;
  self->starts_ = [IOSIntArray newArrayWithLength:((IOSObjectArray *) nil_chk(searchables))->size_ + 1];
...
- (jint)docFreqWithOrgApacheLuceneIndexTerm:(OrgApacheLuceneIndexTerm *)term {
  ...
  @catch (JavaLangNullPointerException *e) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$$", @"df for term ", [((OrgApacheLuceneIndexTerm *) nil_chk(term)) text], @" not available"));
  }
  return df;
}

- (IOSIntArray *)docFreqsWithOrgApacheLuceneIndexTermArray:(IOSObjectArray *)terms {
  IOSIntArray *result = [IOSIntArray newArrayWithLength:((IOSObjectArray *) nil_chk(terms))->size_];
  ...
  for (jint j = 0; j < ((IOSObjectArray *) nil_chk(((OrgApacheLuceneSearchTopFieldDocs *) nil_chk(docs))->fields_))->size_; j++) {
    if ([((OrgApacheLuceneSearchSortField *) nil_chk(IOSObjectArray_Get(docs->fields_, j))) getType] == OrgApacheLuceneSearchSortField_DOC) {
      for (jint j2 = 0; j2 < ((IOSObjectArray *) nil_chk(docs->scoreDocs_))->size_; j2++) {
        ...
        (void) IOSObjectArray_Set(nil_chk(((OrgApacheLuceneSearchFieldDoc *) nil_chk(fd))->fields_), j, JavaLangInteger_valueOfWithInt_([((JavaLangInteger *) nil_chk(((JavaLangInteger *) cast_chk(IOSObjectArray_Get(fd->fields_, j), [JavaLangInteger class])))) intValue] + IOSIntArray_Get(nil_chk(starts_), i_)));
      }
      break;
    }
  }
