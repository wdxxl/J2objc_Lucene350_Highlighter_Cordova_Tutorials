...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/SpanQueryFilter.java
//
...
- (OrgApacheLuceneSearchSpanFilterResult *)bitSpansWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader {
  ...
  id<JavaUtilList> tmp = new_JavaUtilArrayList_initWithInt_(20);
  jint currentDoc = -1;
  ...
  while ([((OrgApacheLuceneSearchSpansSpans *) nil_chk(spans)) next]) {
    ...
    [bits setWithInt:doc];
    if (currentDoc != doc) {
      currentInfo = new_OrgApacheLuceneSearchSpanFilterResult_PositionInfo_initWithInt_(doc);
  ...
  return new_OrgApacheLuceneSearchSpanFilterResult_initWithOrgApacheLuceneSearchDocIdSet_withJavaUtilList_(bits, tmp);
}
