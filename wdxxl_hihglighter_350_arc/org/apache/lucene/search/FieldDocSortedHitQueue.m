...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/FieldDocSortedHitQueue.java
//
...
- (void)setFieldsWithOrgApacheLuceneSearchSortFieldArray:(IOSObjectArray *)fields {
  (void) JreVolatileStrongAssign(&self->fields_, fields);
  (void) JreVolatileStrongAssign(&self->collators_, OrgApacheLuceneSearchFieldDocSortedHitQueue_hasCollatorsWithOrgApacheLuceneSearchSortFieldArray_(self, fields));
  (void) JreVolatileStrongAssign(&comparators_, [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(fields))->size_ type:OrgApacheLuceneSearchFieldComparator_class_()]);
  for (jint fieldIDX = 0; fieldIDX < fields->size_; fieldIDX++) {
    (void) IOSObjectArray_Set(nil_chk(JreLoadVolatileId(&comparators_)), fieldIDX, [((OrgApacheLuceneSearchSortField *) nil_chk(IOSObjectArray_Get(fields, fieldIDX))) getComparatorWithInt:1 withInt:fieldIDX]);
  }
}
...
void OrgApacheLuceneSearchFieldDocSortedHitQueue_initWithInt_(OrgApacheLuceneSearchFieldDocSortedHitQueue *self, jint size) {
  ...
  (void) JreVolatileStrongAssign(&self->fields_, nil);
  (void) JreVolatileStrongAssign(&self->collators_, nil);
  (void) JreVolatileStrongAssign(&self->comparators_, nil);
...
IOSObjectArray *OrgApacheLuceneSearchFieldDocSortedHitQueue_hasCollatorsWithOrgApacheLuceneSearchSortFieldArray_(OrgApacheLuceneSearchFieldDocSortedHitQueue *self, IOSObjectArray *fields) {
  ...
  IOSObjectArray *ret = [IOSObjectArray newArrayWithLength:fields->size_ type:JavaTextCollator_class_()];
  for (jint i = 0; i < fields->size_; ++i) {
    ...
    if (locale != nil) (void) IOSObjectArray_Set(ret, i, JavaTextCollator_getInstanceWithJavaUtilLocale_(locale));
  }
  return ret;
}
