...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/FuzzyTermEnum.java
//
...
void OrgApacheLuceneSearchFuzzyTermEnum_initWithOrgApacheLuceneIndexIndexReader_withOrgApacheLuceneIndexTerm_withFloat_withInt_(OrgApacheLuceneSearchFuzzyTermEnum *self, OrgApacheLuceneIndexIndexReader *reader, OrgApacheLuceneIndexTerm *term, jfloat minSimilarity, jint prefixLength) {
  ...
  self->endEnum_ = false;
  self->searchTerm_ = nil;
  if (minSimilarity >= 1.0f) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"minimumSimilarity cannot be greater than or equal to 1");
  else if (minSimilarity < 0.0f) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"minimumSimilarity cannot be less than 0");
  if (prefixLength < 0) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"prefixLength cannot be less than 0");
  ...
  self->text_ = [((NSString *) nil_chk([((NSString *) nil_chk([((OrgApacheLuceneIndexTerm *) nil_chk(self->searchTerm_)) text])) java_substring:realPrefixLength])) java_toCharArray];
  self->prefix_ = [((NSString *) nil_chk([((OrgApacheLuceneIndexTerm *) nil_chk(self->searchTerm_)) text])) java_substring:0 endIndex:realPrefixLength];
  self->p_ = [IOSIntArray newArrayWithLength:((IOSCharArray *) nil_chk(self->text_))->size_ + 1];
  self->d_ = [IOSIntArray newArrayWithLength:self->text_->size_ + 1];
  [self setEnumWithOrgApacheLuceneIndexTermEnum:[((OrgApacheLuceneIndexIndexReader *) nil_chk(reader)) termsWithOrgApacheLuceneIndexTerm:new_OrgApacheLuceneIndexTerm_initWithNSString_withNSString_([((OrgApacheLuceneIndexTerm *) nil_chk(self->searchTerm_)) field], self->prefix_)]];
}
