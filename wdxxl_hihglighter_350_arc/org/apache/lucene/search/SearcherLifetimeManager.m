...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/SearcherLifetimeManager.java
//
...
- (jlong)recordWithOrgApacheLuceneSearchIndexSearcher:(OrgApacheLuceneSearchIndexSearcher *)searcher {
  ...
  if (tracker == nil) {
    tracker = new_OrgApacheLuceneSearchSearcherLifetimeManager_SearcherTracker_initWithOrgApacheLuceneSearchIndexSearcher_(searcher);
    ...
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@$@", @"the provided searcher has the same underlying reader version yet the searcher instance differs from before (new=", searcher, @" vs old=", tracker->searcher_));
  }
  return version_;
}
...
- (void)pruneWithOrgApacheLuceneSearchSearcherLifetimeManager_Pruner:(id<OrgApacheLuceneSearchSearcherLifetimeManager_Pruner>)pruner {
  @synchronized(self) {
    id<JavaUtilList> trackers = new_JavaUtilArrayList_init();
...
- (void)close {
  @synchronized(self) {
    ...
    id<JavaUtilList> toClose = new_JavaUtilArrayList_initWithJavaUtilCollection_([((JavaUtilConcurrentConcurrentHashMap *) nil_chk(searchers_)) values]);
    for (OrgApacheLuceneSearchSearcherLifetimeManager_SearcherTracker * __strong tracker in toClose) {
      (void) [searchers_ removeWithId:JavaLangLong_valueOfWithLong_(((OrgApacheLuceneSearchSearcherLifetimeManager_SearcherTracker *) nil_chk(tracker))->version__)];
    }
    OrgApacheLuceneUtilIOUtils_closeWithJavaLangIterable_(toClose);
    if ([searchers_ size] != 0) {
      @throw new_JavaLangIllegalStateException_initWithNSString_(@"another thread called record while this SearcherLifetimeManager instance was being closed; not all searchers were closed");
    }
  }
}
...
void OrgApacheLuceneSearchSearcherLifetimeManager_init(OrgApacheLuceneSearchSearcherLifetimeManager *self) {
  NSObject_init(self);
  self->searchers_ = new_JavaUtilConcurrentConcurrentHashMap_init();
}
...
void OrgApacheLuceneSearchSearcherLifetimeManager_ensureOpen(OrgApacheLuceneSearchSearcherLifetimeManager *self) {
  if (JreLoadVolatileBoolean(&self->closed_)) {
    @throw new_OrgApacheLuceneStoreAlreadyClosedException_initWithNSString_(@"this SearcherLifetimeManager instance is closed");
  }
}
...
- (jint)compareToWithId:(OrgApacheLuceneSearchSearcherLifetimeManager_SearcherTracker *)other {
  (void) cast_chk(other, [OrgApacheLuceneSearchSearcherLifetimeManager_SearcherTracker class]);
...
void OrgApacheLuceneSearchSearcherLifetimeManager_PruneByAge_initWithInt_(OrgApacheLuceneSearchSearcherLifetimeManager_PruneByAge *self, jint maxAgeSec) {
  NSObject_init(self);
  if (maxAgeSec < 1) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$IC", @"maxAgeSec must be > 0 (got ", maxAgeSec, ')'));
  }
