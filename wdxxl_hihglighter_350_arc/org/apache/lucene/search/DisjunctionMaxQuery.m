...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/search/DisjunctionMaxQuery.java
//
...
- (OrgApacheLuceneSearchWeight *)createWeightWithOrgApacheLuceneSearchSearcher:(OrgApacheLuceneSearchSearcher *)searcher {
  return new_OrgApacheLuceneSearchDisjunctionMaxQuery_DisjunctionMaxWeight_initWithOrgApacheLuceneSearchDisjunctionMaxQuery_withOrgApacheLuceneSearchSearcher_(self, searcher);
}
...
- (OrgApacheLuceneSearchQuery *)rewriteWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader {
  ...
  for (jint i = 0; i < numDisjunctions; i++) {
    ...
    if (rewrite != clause) {
      ...
      (void) [((JavaUtilArrayList *) nil_chk(((OrgApacheLuceneSearchDisjunctionMaxQuery *) nil_chk(clone))->disjuncts_)) setWithInt:i withId:rewrite];
    }
  }
...
- (id)java_clone {
  ...
  ((OrgApacheLuceneSearchDisjunctionMaxQuery *) nil_chk(clone))->disjuncts_ = (JavaUtilArrayList *) cast_chk([((JavaUtilArrayList *) nil_chk(self->disjuncts_)) java_clone], [JavaUtilArrayList class]);
  return clone;
}
...
- (NSString *)toStringWithNSString:(NSString *)field {
  ...
  for (jint i = 0; i < numDisjunctions; i++) {
    ...
    if ([subquery isKindOfClass:[OrgApacheLuceneSearchBooleanQuery class]]) {
      (void) [buffer appendWithNSString:@"("];
      (void) [buffer appendWithNSString:[((OrgApacheLuceneSearchQuery *) nil_chk(subquery)) toStringWithNSString:field]];
      (void) [buffer appendWithNSString:@")"];
    }
    else (void) [buffer appendWithNSString:[((OrgApacheLuceneSearchQuery *) nil_chk(subquery)) toStringWithNSString:field]];
    if (i != numDisjunctions - 1) (void) [buffer appendWithNSString:@" | "];
  }
  (void) [buffer appendWithNSString:@")"];
  if (tieBreakerMultiplier_ != 0.0f) {
    (void) [buffer appendWithNSString:@"~"];
    (void) [buffer appendWithFloat:tieBreakerMultiplier_];
  }
  if ([self getBoost] != 1.0) {
    (void) [buffer appendWithNSString:@"^"];
    (void) [buffer appendWithFloat:[self getBoost]];
  }
...
void OrgApacheLuceneSearchDisjunctionMaxQuery_initWithFloat_(OrgApacheLuceneSearchDisjunctionMaxQuery *self, jfloat tieBreakerMultiplier) {
  ...
  self->disjuncts_ = new_JavaUtilArrayList_init();
...
- (OrgApacheLuceneSearchScorer *)scorerWithOrgApacheLuceneIndexIndexReader:(OrgApacheLuceneIndexIndexReader *)reader
  ...
  IOSObjectArray *scorers = [IOSObjectArray newArrayWithLength:[((JavaUtilArrayList *) nil_chk(weights_)) size] type:OrgApacheLuceneSearchScorer_class_()];
  jint idx = 0;
  for (OrgApacheLuceneSearchWeight * __strong w in nil_chk(weights_)) {
    ...
    if (subScorer != nil && [subScorer nextDoc] != OrgApacheLuceneSearchDocIdSetIterator_NO_MORE_DOCS) {
      (void) IOSObjectArray_Set(scorers, idx++, subScorer);
    }
  }
  if (idx == 0) return nil;
  OrgApacheLuceneSearchDisjunctionMaxScorer *result = new_OrgApacheLuceneSearchDisjunctionMaxScorer_initWithOrgApacheLuceneSearchWeight_withFloat_withOrgApacheLuceneSearchSimilarity_withOrgApacheLuceneSearchScorerArray_withInt_(self, this$0_->tieBreakerMultiplier_, similarity_, scorers, idx);
  return result;
}
...
void OrgApacheLuceneSearchDisjunctionMaxQuery_DisjunctionMaxWeight_initWithOrgApacheLuceneSearchDisjunctionMaxQuery_withOrgApacheLuceneSearchSearcher_(OrgApacheLuceneSearchDisjunctionMaxQuery_DisjunctionMaxWeight *self, OrgApacheLuceneSearchDisjunctionMaxQuery *outer$, OrgApacheLuceneSearchSearcher *searcher) {
  ...
  self->weights_ = new_JavaUtilArrayList_init();
  self->similarity_ = [((OrgApacheLuceneSearchSearcher *) nil_chk(searcher)) getSimilarity];
