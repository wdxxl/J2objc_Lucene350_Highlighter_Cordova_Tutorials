...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/MMapDirectory.java
//
...
- (void)setUseUnmapWithBoolean:(jboolean)useUnmapHack {
  if (useUnmapHack && !OrgApacheLuceneStoreMMapDirectory_UNMAP_SUPPORTED) @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"Unmap hack not supported on this platform!");
...
- (OrgApacheLuceneStoreIndexInput *)openInputWithNSString:(NSString *)name
  ...
  [self ensureOpen];
  JavaIoFile *f = new_JavaIoFile_initWithJavaIoFile_withNSString_([self getDirectory], name);
  JavaIoRandomAccessFile *raf = new_JavaIoRandomAccessFile_initWithJavaIoFile_withNSString_(f, @"r");
  @try {
    return new_OrgApacheLuceneStoreMMapDirectory_MMapIndexInput_initWithOrgApacheLuceneStoreMMapDirectory_withNSString_withJavaIoRandomAccessFile_withInt_(self, JreStrcat("$@$", @"MMapIndexInput(path=\"", f, @"\")"), raf, chunkSizePower_);
  }
  @finally {
    [raf close];
  }
...
+ (void)initialize {
  if (self == [OrgApacheLuceneStoreMMapDirectory class]) {
    OrgApacheLuceneStoreMMapDirectory_DEFAULT_MAX_BUFF = JreLoadStatic(OrgApacheLuceneUtilConstants, JRE_IS_64BIT) ? (JreLShift32(1, 30)) : (JreLShift32(1, 28));
    {
      jboolean v;
      @try {
        (void) IOSClass_forName_(@"sun.misc.Cleaner");
        (void) [((IOSClass *) nil_chk(IOSClass_forName_(@"java.nio.DirectByteBuffer"))) getMethod:@"cleaner" parameterTypes:[IOSObjectArray newArrayWithLength:0 type:IOSClass_class_()]];
        v = true;
      }
...
void OrgApacheLuceneStoreMMapDirectory_cleanMappingWithJavaNioByteBuffer_(OrgApacheLuceneStoreMMapDirectory *self, JavaNioByteBuffer *buffer) {
  if (self->useUnmapHack_) {
    @try {
      (void) JavaSecurityAccessController_doPrivilegedWithJavaSecurityPrivilegedExceptionAction_(new_OrgApacheLuceneStoreMMapDirectory_1_initWithJavaNioByteBuffer_(buffer));
    }
...
- (id)run {
  JavaLangReflectMethod *getCleanerMethod = [[((JavaNioByteBuffer *) nil_chk(val$buffer_)) java_getClass] getMethod:@"cleaner" parameterTypes:[IOSObjectArray newArrayWithLength:0 type:IOSClass_class_()]];
  [((JavaLangReflectMethod *) nil_chk(getCleanerMethod)) setAccessibleWithBoolean:true];
  id cleaner = [getCleanerMethod invokeWithId:val$buffer_ withNSObjectArray:[IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]];
  if (cleaner != nil) {
    (void) [((JavaLangReflectMethod *) nil_chk([[cleaner java_getClass] getMethod:@"clean" parameterTypes:[IOSObjectArray newArrayWithLength:0 type:IOSClass_class_()]])) invokeWithId:cleaner withNSObjectArray:[IOSObjectArray newArrayWithLength:0 type:NSObject_class_()]];
  }
  return nil;
}
  ...
  @catch (JavaNioBufferUnderflowException *e) {
    do {
      ...
      if (curBufIndex_ >= ((IOSObjectArray *) nil_chk(buffers_))->size_) {
        @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"read past EOF: ", self));
      }
      curBuf_ = IOSObjectArray_Get(buffers_, curBufIndex_);
      (void) [((JavaNioByteBuffer *) nil_chk(curBuf_)) positionWithInt:0];
    }
...
- (void)readBytesWithByteArray:(IOSByteArray *)b
                       ...
                       withInt:(jint)len {
  @try {
    (void) [((JavaNioByteBuffer *) nil_chk(curBuf_)) getWithByteArray:b withInt:offset withInt:len];
  }
  @catch (JavaNioBufferUnderflowException *e) {
    jint curAvail = [((JavaNioByteBuffer *) nil_chk(curBuf_)) remaining];
    while (len > curAvail) {
      (void) [((JavaNioByteBuffer *) nil_chk(curBuf_)) getWithByteArray:b withInt:offset withInt:curAvail];
      len -= curAvail;
      offset += curAvail;
      curBufIndex_++;
      if (curBufIndex_ >= ((IOSObjectArray *) nil_chk(buffers_))->size_) {
...
- (void)seekWithLong:(jlong)pos {
  ...
  @try {
    ...
    (void) [((JavaNioByteBuffer *) nil_chk(b)) positionWithInt:(jint) (pos & chunkSizeMask_)];
    self->curBufIndex_ = bi;
    self->curBuf_ = b;
  }
  @catch (JavaLangArrayIndexOutOfBoundsException *aioobe) {
    if (pos < 0LL) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Seeking to negative position: ", self));
    }
    @throw new_JavaIoIOException_initWithNSString_(@"seek past EOF");
  }
  @catch (JavaLangIllegalArgumentException *iae) {
    if (pos < 0LL) {
      @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$@", @"Seeking to negative position: ", self));
    }
    @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"seek past EOF: ", self));
  }
}
...
- (id)java_clone {
  if (buffers_ == nil) {
    @throw new_OrgApacheLuceneStoreAlreadyClosedException_initWithNSString_(JreStrcat("$@", @"MMapIndexInput already closed: ", self));
  }
  ...
  clone->buffers_ = [IOSObjectArray newArrayWithLength:((IOSObjectArray *) nil_chk(buffers_))->size_ type:JavaNioByteBuffer_class_()];
  for (jint bufNr = 0; bufNr < ((IOSObjectArray *) nil_chk(buffers_))->size_; bufNr++) {
    (void) IOSObjectArray_Set(clone->buffers_, bufNr, [((JavaNioByteBuffer *) nil_chk(IOSObjectArray_Get(buffers_, bufNr))) duplicate]);
  }
  @try {
...
- (void)close {
  @try {
    ...
    for (jint bufNr = 0; bufNr < buffers_->size_; bufNr++) {
      ...
      @finally {
        (void) IOSObjectArray_Set(nil_chk(buffers_), bufNr, nil);
      }
    }
  }
  @finally {
...
void OrgApacheLuceneStoreMMapDirectory_MMapIndexInput_initWithOrgApacheLuceneStoreMMapDirectory_withNSString_withJavaIoRandomAccessFile_withInt_(OrgApacheLuceneStoreMMapDirectory_MMapIndexInput *self, OrgApacheLuceneStoreMMapDirectory *outer$, NSString *resourceDescription, JavaIoRandomAccessFile *raf, jint chunkSizePower) {
  ...
  if (chunkSizePower < 0 || chunkSizePower > 30) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$I", @"Invalid chunkSizePower used for ByteBuffer size: ", chunkSizePower));
  if ((JreURShift64(self->length_, chunkSizePower)) >= JavaLangInteger_MAX_VALUE) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$", @"RandomAccessFile too big for chunk size: ", [raf description]));
  jint nrBuffers = (jint) (JreURShift64(self->length_, chunkSizePower)) + 1;
  self->buffers_ = [IOSObjectArray newArrayWithLength:nrBuffers type:JavaNioByteBuffer_class_()];
  ...
  for (jint bufNr = 0; bufNr < nrBuffers; bufNr++) {
    ...
    (void) IOSObjectArray_Set(nil_chk(self->buffers_), bufNr, [((JavaNioChannelsFileChannel *) nil_chk(rafc)) mapWithJavaNioChannelsFileChannel_MapMode:JreLoadStatic(JavaNioChannelsFileChannel_MapMode, READ_ONLY) withLong:bufferStart withLong:bufSize]);
    bufferStart += bufSize;
  }
