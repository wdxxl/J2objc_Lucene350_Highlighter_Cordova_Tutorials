...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/store/FSDirectory.java
//
...
- (jboolean)fileExistsWithNSString:(NSString *)name {
  [self ensureOpen];
  JavaIoFile *file = new_JavaIoFile_initWithJavaIoFile_withNSString_(directory_, name);
  return [file exists];
}
...
- (jlong)fileLengthWithNSString:(NSString *)name {
  ...
  if (len == 0 && ![file exists]) {
    @throw new_JavaIoFileNotFoundException_initWithNSString_(name);
  }
  else {
    return len;
  }
...
- (void)deleteFileWithNSString:(NSString *)name {
  [self ensureOpen];
  JavaIoFile *file = new_JavaIoFile_initWithJavaIoFile_withNSString_(directory_, name);
  if (![file delete__]) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"Cannot delete ", file));
...
- (OrgApacheLuceneStoreIndexOutput *)createOutputWithNSString:(NSString *)name {
  ...
  return new_OrgApacheLuceneStoreFSDirectory_FSIndexOutput_initWithOrgApacheLuceneStoreFSDirectory_withNSString_(self, name);
}

- (void)ensureCanWriteWithNSString:(NSString *)name {
  if (![((JavaIoFile *) nil_chk(directory_)) exists]) if (![directory_ mkdirs]) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"Cannot create directory: ", directory_));
  JavaIoFile *file = new_JavaIoFile_initWithJavaIoFile_withNSString_(directory_, name);
  if ([file exists] && ![file delete__]) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@", @"Cannot overwrite: ", file));
}
...
- (void)syncWithJavaUtilCollection:(id<JavaUtilCollection>)names {
  [self ensureOpen];
  id<JavaUtilSet> toSync = new_JavaUtilHashSet_initWithJavaUtilCollection_(names);
...
- (void)setReadChunkSizeWithInt:(jint)chunkSize {
  if (chunkSize <= 0) {
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(@"chunkSize must be positive");
  }
...
- (void)fsyncWithNSString:(NSString *)name {
  JavaIoFile *fullFile = new_JavaIoFile_initWithJavaIoFile_withNSString_(directory_, name);
  ...
  while (!success && retryCount < 5) {
    ...
    @try {
      @try {
        file = new_JavaIoRandomAccessFile_initWithJavaIoFile_withNSString_(fullFile, @"rw");
...
JavaIoFile *OrgApacheLuceneStoreFSDirectory_getCanonicalPathWithJavaIoFile_(JavaIoFile *file) {
  ...
  return new_JavaIoFile_initWithNSString_([((JavaIoFile *) nil_chk(file)) getCanonicalPath]);
}
...
void OrgApacheLuceneStoreFSDirectory_initWithJavaIoFile_withOrgApacheLuceneStoreLockFactory_(OrgApacheLuceneStoreFSDirectory *self, JavaIoFile *path, OrgApacheLuceneStoreLockFactory *lockFactory) {
  ...
  self->staleFiles_ = JavaUtilCollections_synchronizedSetWithJavaUtilSet_(new_JavaUtilHashSet_init());
  self->chunkSize_ = OrgApacheLuceneStoreFSDirectory_DEFAULT_READ_CHUNK_SIZE;
  if (lockFactory == nil) {
    lockFactory = new_OrgApacheLuceneStoreNativeFSLockFactory_init();
  }
  self->directory_ = OrgApacheLuceneStoreFSDirectory_getCanonicalPathWithJavaIoFile_(path);
  if ([((JavaIoFile *) nil_chk(self->directory_)) exists] && ![self->directory_ isDirectory]) @throw new_OrgApacheLuceneStoreNoSuchDirectoryException_initWithNSString_(JreStrcat("$@$", @"file '", self->directory_, @"' exists but is not a directory"));
...
OrgApacheLuceneStoreFSDirectory *OrgApacheLuceneStoreFSDirectory_openWithJavaIoFile_withOrgApacheLuceneStoreLockFactory_(JavaIoFile *path, OrgApacheLuceneStoreLockFactory *lockFactory) {
  ...
  if ((JreLoadStatic(OrgApacheLuceneUtilConstants, WINDOWS) || JreLoadStatic(OrgApacheLuceneUtilConstants, SUN_OS) || JreLoadStatic(OrgApacheLuceneUtilConstants, LINUX)) && JreLoadStatic(OrgApacheLuceneUtilConstants, JRE_IS_64BIT) && JreLoadStatic(OrgApacheLuceneStoreMMapDirectory, UNMAP_SUPPORTED)) {
    return new_OrgApacheLuceneStoreMMapDirectory_initWithJavaIoFile_withOrgApacheLuceneStoreLockFactory_(path, lockFactory);
  }
  else if (JreLoadStatic(OrgApacheLuceneUtilConstants, WINDOWS)) {
    return new_OrgApacheLuceneStoreSimpleFSDirectory_initWithJavaIoFile_withOrgApacheLuceneStoreLockFactory_(path, lockFactory);
  }
  else {
    return new_OrgApacheLuceneStoreNIOFSDirectory_initWithJavaIoFile_withOrgApacheLuceneStoreLockFactory_(path, lockFactory);
  }
}

IOSObjectArray *OrgApacheLuceneStoreFSDirectory_listAllWithJavaIoFile_(JavaIoFile *dir) {
  OrgApacheLuceneStoreFSDirectory_initialize();
  if (![((JavaIoFile *) nil_chk(dir)) exists]) @throw new_OrgApacheLuceneStoreNoSuchDirectoryException_initWithNSString_(JreStrcat("$@$", @"directory '", dir, @"' does not exist"));
  else if (![dir isDirectory]) @throw new_OrgApacheLuceneStoreNoSuchDirectoryException_initWithNSString_(JreStrcat("$@$", @"file '", dir, @"' exists but is not a directory"));
  IOSObjectArray *result = [dir listWithJavaIoFilenameFilter:new_OrgApacheLuceneStoreFSDirectory_1_init()];
  if (result == nil) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@$", @"directory '", dir, @"' exists and is a directory, but cannot be listed: list() returned null"));
  return result;
}

jlong OrgApacheLuceneStoreFSDirectory_fileModifiedWithJavaIoFile_withNSString_(JavaIoFile *directory, NSString *name) {
  OrgApacheLuceneStoreFSDirectory_initialize();
  JavaIoFile *file = new_JavaIoFile_initWithJavaIoFile_withNSString_(directory, name);
  return [file lastModified];
}
...
- (jboolean)acceptWithJavaIoFile:(JavaIoFile *)dir
                    withNSString:(NSString *)file {
  return ![new_JavaIoFile_initWithJavaIoFile_withNSString_(dir, file) isDirectory];
}
...
void OrgApacheLuceneStoreFSDirectory_FSIndexOutput_initWithOrgApacheLuceneStoreFSDirectory_withNSString_(OrgApacheLuceneStoreFSDirectory_FSIndexOutput *self, OrgApacheLuceneStoreFSDirectory *parent, NSString *name) {
  ...
  self->parent_ = parent;
  self->name_ = name;
  self->file_ = new_JavaIoRandomAccessFile_initWithJavaIoFile_withNSString_(new_JavaIoFile_initWithJavaIoFile_withNSString_(((OrgApacheLuceneStoreFSDirectory *) nil_chk(parent))->directory_, name), @"rw");
