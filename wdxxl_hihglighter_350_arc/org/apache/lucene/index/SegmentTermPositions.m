...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SegmentTermPositions.java
//
...
- (jint)readWithIntArray:(IOSIntArray *)docs
  ...
  @throw new_JavaLangUnsupportedOperationException_initWithNSString_(@"TermPositions does not support processing multiple documents in one call. Use TermDocs instead.");
}
...
- (IOSByteArray *)getPayloadWithByteArray:(IOSByteArray *)data
  ...
  if (!needToLoadPayload_) {
    @throw new_JavaIoIOException_initWithNSString_(@"Either no payload exists at this term position or an attempt was made to load it more than once.");
  }
  IOSByteArray *retArray;
  jint retOffset;
  if (data == nil || data->size_ - offset < payloadLength_) {
    retArray = [IOSByteArray newArrayWithLength:payloadLength_];
    retOffset = 0;
  }
  else {
    retArray = data;
...
void OrgApacheLuceneIndexSegmentTermPositions_lazySkip(OrgApacheLuceneIndexSegmentTermPositions *self) {
  if (self->proxStream_ == nil) {
    self->proxStream_ = (OrgApacheLuceneStoreIndexInput *) cast_chk([((OrgApacheLuceneStoreIndexInput *) nil_chk(((OrgApacheLuceneIndexSegmentCoreReaders *) nil_chk(((OrgApacheLuceneIndexSegmentReader *) nil_chk(self->parent_))->core_))->proxStream_)) java_clone], [OrgApacheLuceneStoreIndexInput class]);
  }
  OrgApacheLuceneIndexSegmentTermPositions_skipPayload(self);
