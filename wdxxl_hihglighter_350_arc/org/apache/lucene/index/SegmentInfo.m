...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SegmentInfo.java
//
...
- (void)resetWithOrgApacheLuceneIndexSegmentInfo:(OrgApacheLuceneIndexSegmentInfo *)src {
  ...
  version__ = ((OrgApacheLuceneIndexSegmentInfo *) nil_chk(src))->version__;
  name_ = src->name_;
  ...
  if (src->normGen_ == nil) {
    ...
    normGen_ = [IOSLongArray newArrayWithLength:src->normGen_->size_];
...
- (void)setNumFieldsWithInt:(jint)numFields {
  if (normGen_ == nil) {
    normGen_ = [IOSLongArray newArrayWithLength:numFields];
    if (preLockless_) {
    }
    else {
...
- (id)java_clone {
  OrgApacheLuceneIndexSegmentInfo *si = new_OrgApacheLuceneIndexSegmentInfo_initWithNSString_withInt_withOrgApacheLuceneStoreDirectory_withBoolean_withBoolean_withBoolean_withBoolean_(name_, docCount_, dir_, false, hasSingleNormFile_, hasProx_, hasVectors_);
  si->docStoreOffset_ = docStoreOffset_;
  si->docStoreSegment_ = docStoreSegment_;
  ...
  si->diagnostics_ = new_JavaUtilHashMap_initWithJavaUtilMap_(diagnostics_);
  if (normGen_ != nil) {
    si->normGen_ = [normGen_ java_clone];
  }
  si->version__ = version__;
  return si;
}
...
- (jboolean)hasSeparateNorms {
  if (normGen_ == nil) {
    if (!preLockless_) {
      ...
      if (result == nil) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$@$", @"cannot read directory ", dir_, @": listAll() returned null"));
...
- (jint)getDelCount {
  if (delCount_ == -1) {
    if ([self hasDeletions]) {
      ...
      delCount_ = [new_OrgApacheLuceneUtilBitVector_initWithOrgApacheLuceneStoreDirectory_withNSString_(dir_, delFileName) count];
    }
    else delCount_ = 0;
  }
...
- (id<JavaUtilList>)files {
  ...
  JavaUtilHashSet *filesSet = new_JavaUtilHashSet_init();
  ...
  return JreVolatileStrongAssign(&files_, new_JavaUtilArrayList_initWithJavaUtilCollection_(filesSet));
}
...
- (NSString *)toStringWithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)dir
  ...
  (void) [((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([((JavaLangStringBuilder *) nil_chk([s appendWithNSString:name_])) appendWithChar:'('])) appendWithNSString:version__ == nil ? @"?" : version__])) appendWithChar:')'])) appendWithChar:':'];
  jchar cfs;
  @try {
  ...
  (void) [s appendWithChar:cfs];
  if (self->dir_ != dir) {
    (void) [s appendWithChar:'x'];
  }
  if (hasVectors_) {
    (void) [s appendWithChar:'v'];
  }
  (void) [s appendWithInt:docCount_];
  jint delCount;
  @try {
  ...
  if (delCount != 0) {
    ...
    if (delCount == -1) {
      ...
      (void) [s appendWithInt:delCount];
    }
  }
  if (docStoreOffset_ != -1) {
    (void) [((JavaLangStringBuilder *) nil_chk([s appendWithNSString:@"->"])) appendWithNSString:docStoreSegment_];
    if (docStoreIsCompoundFile_) {
      (void) [s appendWithChar:'c'];
    }
    else {
      (void) [s appendWithChar:'C'];
    }
    (void) [((JavaLangStringBuilder *) nil_chk([s appendWithChar:'+'])) appendWithInt:docStoreOffset_];
  }
...
void OrgApacheLuceneIndexSegmentInfo_initWithNSString_withInt_withOrgApacheLuceneStoreDirectory_withBoolean_withBoolean_withBoolean_withBoolean_(OrgApacheLuceneIndexSegmentInfo *self, NSString *name, jint docCount, OrgApacheLuceneStoreDirectory *dir, jboolean isCompoundFile, jboolean hasSingleNormFile, jboolean hasProx, jboolean hasVectors) {
  ...
  self->version__ = JreLoadStatic(OrgApacheLuceneUtilConstants, LUCENE_MAIN_VERSION);
}
...
void OrgApacheLuceneIndexSegmentInfo_initWithOrgApacheLuceneStoreDirectory_withInt_withOrgApacheLuceneStoreIndexInput_(OrgApacheLuceneIndexSegmentInfo *self, OrgApacheLuceneStoreDirectory *dir, jint format, OrgApacheLuceneStoreIndexInput *input) {
  ...
  if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_3_1) {
    self->version__ = [((OrgApacheLuceneStoreIndexInput *) nil_chk(input)) readString];
  }
  self->name_ = [((OrgApacheLuceneStoreIndexInput *) nil_chk(input)) readString];
  ...
  if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_LOCKLESS) {
    ...
    if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_SHARED_DOC_STORE) {
      ...
      if (self->docStoreOffset_ != -1) {
        self->docStoreSegment_ = [input readString];
        self->docStoreIsCompoundFile_ = (1 == [input readByte]);
      }
      else {
        self->docStoreSegment_ = self->name_;
    ...
    if (numNormGen == OrgApacheLuceneIndexSegmentInfo_NO) {
      ...
      self->normGen_ = [IOSLongArray newArrayWithLength:numNormGen];
    ...
    if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_DIAGNOSTICS) {
      self->diagnostics_ = [input readStringStringMap];
    }
    else {
      self->diagnostics_ = JavaUtilCollections_emptyMap();
    }
    if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_HAS_VECTORS) {
      ...
      if (isCompoundFile) {
        dirToTest = new_OrgApacheLuceneIndexCompoundFileReader_initWithOrgApacheLuceneStoreDirectory_withNSString_(dir, OrgApacheLuceneIndexIndexFileNames_segmentFileNameWithNSString_withNSString_(storesSegment, ext));
      }
      else {
        dirToTest = dir;
      }
...
void OrgApacheLuceneIndexSegmentInfo_clearFiles(OrgApacheLuceneIndexSegmentInfo *self) {
  (void) JreVolatileStrongAssign(&self->files_, nil);
  JreAssignVolatileLong(&self->sizeInBytesNoStore_, -1);
  JreAssignVolatileLong(&self->sizeInBytesWithStore_, -1);
}
