...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/FormatPostingsDocsWriter.java
//
...
- (OrgApacheLuceneIndexFormatPostingsPositionsConsumer *)addDocWithInt:(jint)docID
  ...
  if (docID < 0 || (df_ > 0 && delta <= 0)) @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$I$I$@C", @"docs out of order (", docID, @" <= ", lastDocID_, @" ) (out: ", out_, ')'));
...
- (void)close {
  OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ out_, posWriter_ } count:2 type:JavaIoCloseable_class_()]);
}
...
void OrgApacheLuceneIndexFormatPostingsDocsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFormatPostingsTermsWriter_(OrgApacheLuceneIndexFormatPostingsDocsWriter *self, OrgApacheLuceneIndexSegmentWriteState *state, OrgApacheLuceneIndexFormatPostingsTermsWriter *parent) {
  ...
  self->utf8_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF8Result_init();
  self->parent_ = parent;
  self->out_ = [((OrgApacheLuceneStoreDirectory *) nil_chk(((OrgApacheLuceneIndexFormatPostingsFieldsWriter *) nil_chk(((OrgApacheLuceneIndexFormatPostingsTermsWriter *) nil_chk(parent))->parent_))->dir_)) createOutputWithNSString:OrgApacheLuceneIndexIndexFileNames_segmentFileNameWithNSString_withNSString_(parent->parent_->segment_, OrgApacheLuceneIndexIndexFileNames_FREQ_EXTENSION)];
  ...
  @try {
    ...
    self->skipListWriter_ = parent->parent_->skipListWriter_;
    ...
    self->posWriter_ = new_OrgApacheLuceneIndexFormatPostingsPositionsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFormatPostingsDocsWriter_(state, self);
    success = true;
  }
  @finally {
    if (!success) {
      OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ self->out_ } count:1 type:JavaIoCloseable_class_()]);
    }
  }
}
