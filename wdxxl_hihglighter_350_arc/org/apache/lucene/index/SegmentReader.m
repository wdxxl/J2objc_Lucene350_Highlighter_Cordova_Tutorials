...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SegmentReader.java
//
...
- (IOSByteArray *)cloneNormBytesWithByteArray:(IOSByteArray *)bytes {
  IOSByteArray *cloneBytes = [IOSByteArray newArrayWithLength:((IOSByteArray *) nil_chk(bytes))->size_];
...
- (OrgApacheLuceneIndexSegmentReader *)reopenSegmentWithOrgApacheLuceneIndexSegmentInfo:(OrgApacheLuceneIndexSegmentInfo *)si
  ...
  @synchronized(self) {
    ...
    IOSBooleanArray *fieldNormsChanged = [IOSBooleanArray newArrayWithLength:[((OrgApacheLuceneIndexFieldInfos *) nil_chk(((OrgApacheLuceneIndexSegmentCoreReaders *) nil_chk(core_))->fieldInfos_)) size]];
    ...
    OrgApacheLuceneIndexSegmentReader *clone = openReadOnly ? new_OrgApacheLuceneIndexReadOnlySegmentReader_init() : new_OrgApacheLuceneIndexSegmentReader_init();
    ...
    @try {
      ...
      (void) JreVolatileStrongAssign(&clone->readerFinishedListeners_, JreLoadVolatileId(&readerFinishedListeners_));
      ...
      if (doClone) {
        if (deletedDocs_ != nil) {
          ...
          clone->deletedDocs_ = deletedDocs_;
          clone->deletedDocsRef_ = deletedDocsRef_;
        }
      }
      else {
      ...
      clone->norms_ = new_JavaUtilHashMap_init();
      for (jint i = 0; i < fieldNormsChanged->size_; i++) {
        if (doClone || !IOSBooleanArray_Get(fieldNormsChanged, i)) {
          ...
          if (norm != nil) (void) [((id<JavaUtilMap>) nil_chk(clone->norms_)) putWithId:curField withId:(OrgApacheLuceneIndexSegmentNorms *) cast_chk([norm java_clone], [OrgApacheLuceneIndexSegmentNorms class])];
        }
      }
...
- (void)doDeleteWithInt:(jint)docNum {
  if (deletedDocs_ == nil) {
    deletedDocs_ = new_OrgApacheLuceneUtilBitVector_initWithInt_([self maxDoc]);
    deletedDocsRef_ = new_JavaUtilConcurrentAtomicAtomicInteger_initWithInt_(1);
  }
  if ([((JavaUtilConcurrentAtomicAtomicInteger *) nil_chk(deletedDocsRef_)) get] > 1) {
    ...
    deletedDocs_ = [self cloneDeletedDocsWithOrgApacheLuceneUtilBitVector:deletedDocs_];
    deletedDocsRef_ = new_JavaUtilConcurrentAtomicAtomicInteger_initWithInt_(1);
...
- (id<JavaUtilList>)files {
  return new_JavaUtilArrayList_initWithJavaUtilCollection_([((OrgApacheLuceneIndexSegmentInfo *) nil_chk(si_)) files]);
}
...
- (id<OrgApacheLuceneIndexTermDocs>)termDocsWithOrgApacheLuceneIndexTerm:(OrgApacheLuceneIndexTerm *)term {
  if (term == nil) {
    return new_OrgApacheLuceneIndexAllTermDocs_initWithOrgApacheLuceneIndexSegmentReader_(self);
  }
  else {
...
- (id<OrgApacheLuceneIndexTermDocs>)termDocs {
  [self ensureOpen];
  return new_OrgApacheLuceneIndexSegmentTermDocs_initWithOrgApacheLuceneIndexSegmentReader_(self);
}

- (id<OrgApacheLuceneIndexTermPositions>)termPositions {
  [self ensureOpen];
  return new_OrgApacheLuceneIndexSegmentTermPositions_initWithOrgApacheLuceneIndexSegmentReader_(self);
}
...
- (void)doSetNormWithInt:(jint)doc
  ...
  OrgApacheLuceneIndexSegmentNorms *norm = [((id<JavaUtilMap>) nil_chk(norms_)) getWithId:field];
  if (norm == nil) {
    @throw new_JavaLangIllegalStateException_initWithNSString_(JreStrcat("$$$", @"Cannot setNorm for field ", field, @": norms were omitted"));
  }
  normsDirty_ = true;
...
- (NSString *)description {
  ...
  (void) [buffer appendWithNSString:[((OrgApacheLuceneIndexSegmentInfo *) nil_chk(si_)) toStringWithOrgApacheLuceneStoreDirectory:((OrgApacheLuceneIndexSegmentCoreReaders *) nil_chk(core_))->dir_ withInt:pendingDeleteCount_]];
...
- (void)startCommit {
  rollbackSegmentInfo_ = (OrgApacheLuceneIndexSegmentInfo *) cast_chk([((OrgApacheLuceneIndexSegmentInfo *) nil_chk(si_)) java_clone], [OrgApacheLuceneIndexSegmentInfo class]);
...
void OrgApacheLuceneIndexSegmentReader_init(OrgApacheLuceneIndexSegmentReader *self) {
  ...
  self->fieldsReaderLocal_ = new_OrgApacheLuceneIndexSegmentReader_FieldsReaderLocal_initWithOrgApacheLuceneIndexSegmentReader_(self);
  self->termVectorsLocal_ = new_OrgApacheLuceneUtilCloseableThreadLocal_init();
  self->deletedDocs_ = nil;
  self->deletedDocsRef_ = nil;
  ...
  self->norms_ = new_JavaUtilHashMap_init();
}
...
OrgApacheLuceneIndexSegmentReader *OrgApacheLuceneIndexSegmentReader_getWithBoolean_withOrgApacheLuceneStoreDirectory_withOrgApacheLuceneIndexSegmentInfo_withInt_withBoolean_withInt_(jboolean readOnly, OrgApacheLuceneStoreDirectory *dir, OrgApacheLuceneIndexSegmentInfo *si, jint readBufferSize, jboolean doOpenStores, jint termInfosIndexDivisor) {
  OrgApacheLuceneIndexSegmentReader_initialize();
  OrgApacheLuceneIndexSegmentReader *instance = readOnly ? new_OrgApacheLuceneIndexReadOnlySegmentReader_init() : new_OrgApacheLuceneIndexSegmentReader_init();
  instance->readOnly_ = readOnly;
  instance->si_ = si;
  instance->readBufferSize_ = readBufferSize;
  jboolean success = false;
  @try {
    instance->core_ = new_OrgApacheLuceneIndexSegmentCoreReaders_initWithOrgApacheLuceneIndexSegmentReader_withOrgApacheLuceneStoreDirectory_withOrgApacheLuceneIndexSegmentInfo_withInt_withInt_(instance, dir, si, readBufferSize, termInfosIndexDivisor);
    if (doOpenStores) {
...
void OrgApacheLuceneIndexSegmentReader_loadDeletedDocs(OrgApacheLuceneIndexSegmentReader *self) {
  if (OrgApacheLuceneIndexSegmentReader_hasDeletionsWithOrgApacheLuceneIndexSegmentInfo_(self->si_)) {
    self->deletedDocs_ = new_OrgApacheLuceneUtilBitVector_initWithOrgApacheLuceneStoreDirectory_withNSString_([self directory], [((OrgApacheLuceneIndexSegmentInfo *) nil_chk(self->si_)) getDelFileName]);
    self->deletedDocsRef_ = new_JavaUtilConcurrentAtomicAtomicInteger_initWithInt_(1);
    ...
    if ([((OrgApacheLuceneUtilBitVector *) nil_chk(self->deletedDocs_)) size] != ((OrgApacheLuceneIndexSegmentInfo *) nil_chk(self->si_))->docCount_) {
      @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$I$I$$", @"document count mismatch: deleted docs count ", [((OrgApacheLuceneUtilBitVector *) nil_chk(self->deletedDocs_)) size], @" vs segment doc count ", ((OrgApacheLuceneIndexSegmentInfo *) nil_chk(self->si_))->docCount_, @" segment=", self->si_->name_));
    }
  }
    ...
    if (fi->isIndexed_ && !fi->omitNorms_) {
      ...
      if (singleNormFile) {
        ...
        if (self->singleNormStream_ == nil) {
          self->singleNormStream_ = [((OrgApacheLuceneStoreDirectory *) nil_chk(d)) openInputWithNSString:fileName withInt:readBufferSize];
          self->singleNormRef_ = new_JavaUtilConcurrentAtomicAtomicInteger_initWithInt_(1);
        }
        else {
      ...
      (void) [((id<JavaUtilMap>) nil_chk(self->norms_)) putWithId:fi->name_ withId:new_OrgApacheLuceneIndexSegmentNorms_initWithOrgApacheLuceneStoreIndexInput_withInt_withLong_withOrgApacheLuceneIndexSegmentReader_(normInput, fi->number_, normSeek, self)];
      nextNormSeek += maxDoc;
    }
  }
}
...
OrgApacheLuceneIndexSegmentReader *OrgApacheLuceneIndexSegmentReader_getOnlySegmentReaderWithOrgApacheLuceneIndexIndexReader_(OrgApacheLuceneIndexIndexReader *reader) {
  ...
  if ([reader isKindOfClass:[OrgApacheLuceneIndexDirectoryReader class]]) {
    ...
    if (((IOSObjectArray *) nil_chk(subReaders))->size_ != 1) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$I$", reader, @" has ", subReaders->size_, @" segments instead of exactly one"));
  ...
  @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("@$", reader, @" is not a SegmentReader or a single-segment DirectoryReader"));
}
