...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/PositionBasedTermVectorMapper.java
//
...
withOrgApacheLuceneIndexTermVectorOffsetInfoArray:(IOSObjectArray *)offsets
  ...
  for (jint i = 0; i < ((IOSIntArray *) nil_chk(positions))->size_; i++) {
    ...
    if (pos == nil) {
      pos = new_OrgApacheLuceneIndexPositionBasedTermVectorMapper_TVPositionInfo_initWithInt_withBoolean_(IOSIntArray_Get(positions, i), storeOffsets_);
      (void) [((id<JavaUtilMap>) nil_chk(currentPositions_)) putWithId:posVal withId:pos];
    }
...
- (void)setExpectationsWithNSString:(NSString *)field
  ...
  if (storePositions == false) {
    @throw new_JavaLangRuntimeException_initWithNSString_(@"You must store positions in order to use this Mapper");
  }
  if (storeOffsets == true) {
  }
  fieldToTerms_ = new_JavaUtilHashMap_initWithInt_(numTerms);
  self->storeOffsets_ = storeOffsets;
  currentField_ = field;
  currentPositions_ = new_JavaUtilHashMap_init();
  (void) [((id<JavaUtilMap>) nil_chk(fieldToTerms_)) putWithId:currentField_ withId:currentPositions_];
}
...
void OrgApacheLuceneIndexPositionBasedTermVectorMapper_TVPositionInfo_initWithInt_withBoolean_(OrgApacheLuceneIndexPositionBasedTermVectorMapper_TVPositionInfo *self, jint position, jboolean storeOffsets) {
  ...
  self->terms_ = new_JavaUtilArrayList_init();
  if (storeOffsets) {
    self->offsets_ = new_JavaUtilArrayList_init();
  }
}
