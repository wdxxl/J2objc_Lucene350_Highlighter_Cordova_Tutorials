...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/CompoundFileWriter.java
//
...
withOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)dir {
  if (merged_) @throw new_JavaLangIllegalStateException_initWithNSString_(@"Can't add extensions after merge has been called");
  if (file == nil) @throw new_JavaLangNullPointerException_initWithNSString_(@"file cannot be null");
  if (![((JavaUtilHashSet *) nil_chk(ids_)) addWithId:file]) @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$$", @"File ", file, @" already added"));
  OrgApacheLuceneIndexCompoundFileWriter_FileEntry *entry_ = new_OrgApacheLuceneIndexCompoundFileWriter_FileEntry_init();
  entry_->file_ = file;
  entry_->dir_ = dir;
...
- (void)close {
  if (merged_) @throw new_JavaLangIllegalStateException_initWithNSString_(@"Merge already performed");
  if ([((JavaUtilLinkedList *) nil_chk(entries_)) isEmpty]) @throw new_JavaLangIllegalStateException_initWithNSString_(@"No entries to merge have been defined");
  merged_ = true;
  ...
  @finally {
    OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaLangException_withJavaIoCloseableArray_(priorException, [IOSObjectArray newArrayWithObjects:(id[]){ os } count:1 type:JavaIoCloseable_class_()]);
  }
}
...
void OrgApacheLuceneIndexCompoundFileWriter_initWithOrgApacheLuceneStoreDirectory_withNSString_withOrgApacheLuceneIndexSegmentMerger_CheckAbort_(OrgApacheLuceneIndexCompoundFileWriter *self, OrgApacheLuceneStoreDirectory *dir, NSString *name, OrgApacheLuceneIndexSegmentMerger_CheckAbort *checkAbort) {
  NSObject_init(self);
  self->merged_ = false;
  if (dir == nil) @throw new_JavaLangNullPointerException_initWithNSString_(@"directory cannot be null");
  if (name == nil) @throw new_JavaLangNullPointerException_initWithNSString_(@"name cannot be null");
  self->checkAbort_ = checkAbort;
  self->directory_ = dir;
  self->fileName_ = name;
  self->ids_ = new_JavaUtilHashSet_init();
...
void OrgApacheLuceneIndexCompoundFileWriter_copyFileWithOrgApacheLuceneIndexCompoundFileWriter_FileEntry_withOrgApacheLuceneStoreIndexOutput_(OrgApacheLuceneIndexCompoundFileWriter *self, OrgApacheLuceneIndexCompoundFileWriter_FileEntry *source, OrgApacheLuceneStoreIndexOutput *os) {
  ...
  @try {
    ...
    if (diff != length) @throw new_JavaIoIOException_initWithNSString_(JreStrcat("$J$J", @"Difference in the output file offsets ", diff, @" does not match the original file length ", length));
  }
  @finally {
