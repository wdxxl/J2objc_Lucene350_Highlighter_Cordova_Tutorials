...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/TermBuffer.java
//
...
- (void)readWithOrgApacheLuceneStoreIndexInput:(OrgApacheLuceneStoreIndexInput *)input
  ...
  self->field_ = [((OrgApacheLuceneIndexFieldInfos *) nil_chk(fieldInfos)) fieldNameWithInt:[input readVInt]];
}
...
- (OrgApacheLuceneIndexTerm *)toTerm {
  ...
  if (term_ == nil) term_ = new_OrgApacheLuceneIndexTerm_initWithNSString_withNSString_withBoolean_(field_, [NSString java_stringWithCharacters:((OrgApacheLuceneUtilUnicodeUtil_UTF16Result *) nil_chk(text_))->result_ offset:0 length:text_->length_], false);
  return term_;
}
...
- (id)java_clone {
  ...
  clone->bytes_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF8Result_init();
  clone->text_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF16Result_init();
...
void OrgApacheLuceneIndexTermBuffer_init(OrgApacheLuceneIndexTermBuffer *self) {
  NSObject_init(self);
  self->text_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF16Result_init();
  self->bytes_ = new_OrgApacheLuceneUtilUnicodeUtil_UTF8Result_init();
}
