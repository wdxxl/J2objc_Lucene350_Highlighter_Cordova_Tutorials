...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/BufferedDeletesStream.java
//
...
- (OrgApacheLuceneIndexBufferedDeletesStream_ApplyDeletesResult *)applyDeletesWithOrgApacheLuceneIndexIndexWriter_ReaderPool:(OrgApacheLuceneIndexIndexWriter_ReaderPool *)readerPool
  ...
  @synchronized(self) {
    ...
    id<JavaUtilList> infos2 = new_JavaUtilArrayList_init();
    ...
    while (infosIDX >= 0) {
      ...
      if (packet != nil && segGen < packet->gen_) {
        if (coalescedDeletes == nil) {
          coalescedDeletes = new_OrgApacheLuceneIndexCoalescedDeletes_init();
        }
        ...
        if (segAllDeletes) {
          if (allDeleted == nil) {
            allDeleted = new_JavaUtilArrayList_init();
          }
    ...
    return new_OrgApacheLuceneIndexBufferedDeletesStream_ApplyDeletesResult_initWithBoolean_withLong_withJavaUtilList_(anyNewDeletes, nextGen_++, allDeleted);
  }
}
...
+ (void)initialize {
  if (self == [OrgApacheLuceneIndexBufferedDeletesStream class]) {
    OrgApacheLuceneIndexBufferedDeletesStream_sortByDelGen = new_OrgApacheLuceneIndexBufferedDeletesStream_1_init();
...
void OrgApacheLuceneIndexBufferedDeletesStream_initWithInt_(OrgApacheLuceneIndexBufferedDeletesStream *self, jint messageID) {
  NSObject_init(self);
  self->deletes_ = new_JavaUtilArrayList_init();
  self->nextGen_ = 1;
  self->bytesUsed_ = new_JavaUtilConcurrentAtomicAtomicLong_init();
  self->numTerms_ = new_JavaUtilConcurrentAtomicAtomicInteger_init();
...
void OrgApacheLuceneIndexBufferedDeletesStream_messageWithNSString_(OrgApacheLuceneIndexBufferedDeletesStream *self, NSString *message) {
  @synchronized(self) {
    if (self->infoStream_ != nil) {
      [self->infoStream_ printlnWithNSString:JreStrcat("$I$@$$$$", @"BD ", self->messageID_, @" [", new_JavaUtilDate_init(), @"; ", [((JavaLangThread *) nil_chk(JavaLangThread_currentThread())) getName], @"]: ", message)];
    }
  }
}
...
jlong OrgApacheLuceneIndexBufferedDeletesStream_applyQueryDeletesWithJavaLangIterable_withOrgApacheLuceneIndexSegmentReader_(OrgApacheLuceneIndexBufferedDeletesStream *self, id<JavaLangIterable> queriesIter, OrgApacheLuceneIndexSegmentReader *reader) {
  @synchronized(self) {
    ...
    for (OrgApacheLuceneIndexBufferedDeletesStream_QueryAndLimit * __strong ent in nil_chk(queriesIter)) {
      ...
      OrgApacheLuceneSearchDocIdSet *docs = [new_OrgApacheLuceneSearchQueryWrapperFilter_initWithOrgApacheLuceneSearchQuery_(query) getDocIdSetWithOrgApacheLuceneIndexIndexReader:reader];
      if (docs != nil) {
