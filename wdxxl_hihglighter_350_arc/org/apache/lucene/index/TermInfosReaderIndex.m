...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/TermInfosReaderIndex.java
//
...
- (void)seekEnumWithOrgApacheLuceneIndexSegmentTermEnum:(OrgApacheLuceneIndexSegmentTermEnum *)enumerator
  ...
  OrgApacheLuceneIndexTermInfo *termInfo = new_OrgApacheLuceneIndexTermInfo_init();
...
- (jint)getIndexOffsetWithOrgApacheLuceneIndexTerm:(OrgApacheLuceneIndexTerm *)term
  ...
  OrgApacheLuceneUtilPagedBytes_PagedBytesDataInput *input = (OrgApacheLuceneUtilPagedBytes_PagedBytesDataInput *) cast_chk([((OrgApacheLuceneUtilPagedBytes_PagedBytesDataInput *) nil_chk(dataInput_)) java_clone], [OrgApacheLuceneUtilPagedBytes_PagedBytesDataInput class]);
  OrgApacheLuceneUtilBytesRef *scratch = new_OrgApacheLuceneUtilBytesRef_init();
  while (hi >= lo) {
...
void OrgApacheLuceneIndexTermInfosReaderIndex_initWithOrgApacheLuceneIndexSegmentTermEnum_withInt_withLong_withInt_(OrgApacheLuceneIndexTermInfosReaderIndex *self, OrgApacheLuceneIndexSegmentTermEnum *indexEnum, jint indexDivisor, jlong tiiFileLength, jint totalIndexInterval) {
  NSObject_init(self);
  self->comparator_ = OrgApacheLuceneUtilBytesRef_getUTF8SortedAsUTF16Comparator();
  ...
  OrgApacheLuceneUtilPagedBytes *dataPagedBytes = new_OrgApacheLuceneUtilPagedBytes_initWithInt_(OrgApacheLuceneIndexTermInfosReaderIndex_estimatePageBitsWithLong_(initialSize));
  OrgApacheLuceneUtilPagedBytes_PagedBytesDataOutput *dataOutput = [dataPagedBytes getDataOutput];
  OrgApacheLuceneUtilPackedGrowableWriter *indexToTerms = new_OrgApacheLuceneUtilPackedGrowableWriter_initWithInt_withInt_withBoolean_(4, self->indexSize_, false);
  NSString *currentField = nil;
  id<JavaUtilList> fieldStrs = new_JavaUtilArrayList_init();
  jint fieldCounter = -1;
  ...
  self->fields_ = [IOSObjectArray newArrayWithLength:[fieldStrs size] type:OrgApacheLuceneIndexTerm_class_()];
  for (jint i = 0; i < ((IOSObjectArray *) nil_chk(self->fields_))->size_; i++) {
    (void) IOSObjectArray_SetAndConsume(self->fields_, i, new_OrgApacheLuceneIndexTerm_initWithNSString_([fieldStrs getWithInt:i]));
  }
  (void) [dataPagedBytes freezeWithBoolean:true];
  self->dataInput_ = [dataPagedBytes getDataInput];
  self->indexToDataOffset_ = [indexToTerms getMutable];
}
