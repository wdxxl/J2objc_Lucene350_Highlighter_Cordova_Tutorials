...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SegmentMerger.java
//
...
void OrgApacheLuceneIndexSegmentMerger_initWithOrgApacheLuceneStoreDirectory_withInt_withNSString_withOrgApacheLuceneIndexMergePolicy_OneMerge_withOrgApacheLuceneIndexPayloadProcessorProvider_withOrgApacheLuceneIndexFieldInfos_(OrgApacheLuceneIndexSegmentMerger *self, OrgApacheLuceneStoreDirectory *dir, jint termIndexInterval, NSString *name, OrgApacheLuceneIndexMergePolicy_OneMerge *merge, OrgApacheLuceneIndexPayloadProcessorProvider *payloadProcessorProvider, OrgApacheLuceneIndexFieldInfos *fieldInfos) {
  ...
  self->queue_ = nil;
  self->payloadProcessorProvider_ = payloadProcessorProvider;
  self->directory_ = dir;
  self->fieldInfos_ = fieldInfos;
  self->segment_ = name;
  if (merge != nil) {
    self->checkAbort_ = new_OrgApacheLuceneIndexSegmentMerger_CheckAbort_initWithOrgApacheLuceneIndexMergePolicy_OneMerge_withOrgApacheLuceneStoreDirectory_(merge, self->directory_);
  }
  else {
    self->checkAbort_ = new_OrgApacheLuceneIndexSegmentMerger_1_initWithOrgApacheLuceneIndexMergePolicy_OneMerge_withOrgApacheLuceneStoreDirectory_(nil, nil);
  }
...
void OrgApacheLuceneIndexSegmentMerger_setMatchingSegmentReaders(OrgApacheLuceneIndexSegmentMerger *self) {
  ...
  self->matchingSegmentReaders_ = [IOSObjectArray newArrayWithLength:numReaders type:OrgApacheLuceneIndexSegmentReader_class_()];
  for (jint i = 0; i < numReaders; i++) {
    ...
    if ([reader isKindOfClass:[OrgApacheLuceneIndexSegmentReader class]]) {
      ...
      if (same) {
        (void) IOSObjectArray_Set(nil_chk(self->matchingSegmentReaders_), i, segmentReader);
        self->matchedCount_++;
      }
    }
  }
  self->rawDocLengths_ = [IOSIntArray newArrayWithLength:OrgApacheLuceneIndexSegmentMerger_MAX_RAW_MERGE_DOCS];
  self->rawDocLengths2_ = [IOSIntArray newArrayWithLength:OrgApacheLuceneIndexSegmentMerger_MAX_RAW_MERGE_DOCS];
}
...
jint OrgApacheLuceneIndexSegmentMerger_mergeFields(OrgApacheLuceneIndexSegmentMerger *self) {
  for (OrgApacheLuceneIndexIndexReader * __strong reader in nil_chk(self->readers_)) {
    if ([reader isKindOfClass:[OrgApacheLuceneIndexSegmentReader class]]) {
      ...
      for (jint j = 0; j < numReaderFieldInfos; j++) {
        (void) [((OrgApacheLuceneIndexFieldInfos *) nil_chk(self->fieldInfos_)) addWithOrgApacheLuceneIndexFieldInfo:[readerFieldInfos fieldInfoWithInt:j]];
      }
    }
    else {
  ...
  jint docCount = 0;
  OrgApacheLuceneIndexSegmentMerger_setMatchingSegmentReaders(self);
  OrgApacheLuceneIndexFieldsWriter *fieldsWriter = new_OrgApacheLuceneIndexFieldsWriter_initWithOrgApacheLuceneStoreDirectory_withNSString_withOrgApacheLuceneIndexFieldInfos_(self->directory_, self->segment_, self->fieldInfos_);
  @try {
    jint idx = 0;
    for (OrgApacheLuceneIndexIndexReader * __strong reader in nil_chk(self->readers_)) {
  ...
  if (4 + ((jlong) docCount) * 8 != fdxFileLength) @throw new_JavaLangRuntimeException_initWithNSString_(JreStrcat("$I$J$$$Z$", @"mergeFields produced an invalid result: docCount is ", docCount, @" but fdx file size is ", fdxFileLength, @" file=", fileName, @" file exists?=", [((OrgApacheLuceneStoreDirectory *) nil_chk(self->directory_)) fileExistsWithNSString:fileName], @"; now aborting this merge to prevent index corruption"));
  self->segmentWriteState_ = new_OrgApacheLuceneIndexSegmentWriteState_initWithJavaIoPrintStream_withOrgApacheLuceneStoreDirectory_withNSString_withOrgApacheLuceneIndexFieldInfos_withInt_withInt_withOrgApacheLuceneIndexBufferedDeletes_(nil, self->directory_, self->segment_, self->fieldInfos_, docCount, self->termIndexInterval_, nil);
  return docCount;
}
...
void OrgApacheLuceneIndexSegmentMerger_mergeVectors(OrgApacheLuceneIndexSegmentMerger *self) {
  OrgApacheLuceneIndexTermVectorsWriter *termVectorsWriter = new_OrgApacheLuceneIndexTermVectorsWriter_initWithOrgApacheLuceneStoreDirectory_withNSString_withOrgApacheLuceneIndexFieldInfos_(self->directory_, self->segment_, self->fieldInfos_);
  @try {
    jint idx = 0;
    for (OrgApacheLuceneIndexIndexReader * __strong reader in nil_chk(self->readers_)) {
      OrgApacheLuceneIndexSegmentReader *matchingSegmentReader = IOSObjectArray_Get(nil_chk(self->matchingSegmentReaders_), idx++);
  ...
  if (4 + ((jlong) self->mergedDocs_) * 16 != tvxSize) @throw new_JavaLangRuntimeException_initWithNSString_(JreStrcat("$I$J$$$Z$", @"mergeVectors produced an invalid result: mergedDocs is ", self->mergedDocs_, @" but tvx size is ", tvxSize, @" file=", fileName, @" file exists?=", [((OrgApacheLuceneStoreDirectory *) nil_chk(self->directory_)) fileExistsWithNSString:fileName], @"; now aborting this merge to prevent index corruption"));
}
...
void OrgApacheLuceneIndexSegmentMerger_mergeTerms(OrgApacheLuceneIndexSegmentMerger *self) {
  OrgApacheLuceneIndexFormatPostingsFieldsConsumer *fieldsConsumer = new_OrgApacheLuceneIndexFormatPostingsFieldsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFieldInfos_(self->segmentWriteState_, self->fieldInfos_);
  @try {
    self->queue_ = new_OrgApacheLuceneIndexSegmentMergeQueue_initWithInt_([((id<JavaUtilList>) nil_chk(self->readers_)) size]);
...
void OrgApacheLuceneIndexSegmentMerger_mergeTermInfosWithOrgApacheLuceneIndexFormatPostingsFieldsConsumer_(OrgApacheLuceneIndexSegmentMerger *self, OrgApacheLuceneIndexFormatPostingsFieldsConsumer *consumer) {
  ...
  for (jint i = 0; i < readerCount; i++) {
    ...
    OrgApacheLuceneIndexSegmentMergeInfo *smi = new_OrgApacheLuceneIndexSegmentMergeInfo_initWithInt_withOrgApacheLuceneIndexTermEnum_withOrgApacheLuceneIndexIndexReader_(base, termEnum, reader);
    if (self->payloadProcessorProvider_ != nil) {
      smi->dirPayloadProcessor_ = [self->payloadProcessorProvider_ getDirProcessorWithOrgApacheLuceneStoreDirectory:[reader directory]];
    }
    IOSIntArray *docMap = [smi getDocMap];
    if (docMap != nil) {
      if (self->docMaps_ == nil) {
        self->docMaps_ = [IOSObjectArray newArrayWithLength:readerCount type:IOSClass_intArray(1)];
      }
  ...
  IOSObjectArray *match = [IOSObjectArray newArrayWithLength:[((id<JavaUtilList>) nil_chk(self->readers_)) size] type:OrgApacheLuceneIndexSegmentMergeInfo_class_()];
  ...
  while ([((OrgApacheLuceneIndexSegmentMergeQueue *) nil_chk(self->queue_)) size] > 0) {
    jint matchSize = 0;
    (void) IOSObjectArray_Set(match, matchSize++, [((OrgApacheLuceneIndexSegmentMergeQueue *) nil_chk(self->queue_)) pop]);
    ...
    while (matchSize > 0) {
      ...
      if ([((OrgApacheLuceneIndexSegmentMergeInfo *) nil_chk(smi)) next]) (void) [((OrgApacheLuceneIndexSegmentMergeQueue *) nil_chk(self->queue_)) addWithId:smi];
      else [smi close];
    }
  }
}
    ...
    while ([postings next]) {
      ...
      if (self->indexOptions_ == JreLoadEnum(OrgApacheLuceneIndexFieldInfo_IndexOptions, DOCS_AND_FREQS_AND_POSITIONS)) {
        for (jint j = 0; j < freq; j++) {
          ...
          if (payloadLength > 0) {
            if (self->payloadBuffer_ == nil || self->payloadBuffer_->size_ < payloadLength) self->payloadBuffer_ = [IOSByteArray newArrayWithLength:payloadLength];
            (void) [postings getPayloadWithByteArray:self->payloadBuffer_ withInt:0];
            if (payloadProcessor != nil) {
              self->payloadBuffer_ = [payloadProcessor processPayloadWithByteArray:self->payloadBuffer_ withInt:0 withInt:payloadLength];
...
void OrgApacheLuceneIndexSegmentMerger_mergeNorms(OrgApacheLuceneIndexSegmentMerger *self) {
  ...
  @try {
    ...
    for (jint i = 0; i < numFieldInfos; i++) {
      ...
      if (((OrgApacheLuceneIndexFieldInfo *) nil_chk(fi))->isIndexed_ && !fi->omitNorms_) {
        ...
        if (normBuffer == nil) {
          normBuffer = [IOSByteArray newArrayWithLength:bufferSize];
        }
        for (OrgApacheLuceneIndexIndexReader * __strong reader in nil_chk(self->readers_)) {
          jint maxDoc = [((OrgApacheLuceneIndexIndexReader *) nil_chk(reader)) maxDoc];
  ...
  @finally {
    if (success) {
      OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ output } count:1 type:JavaIoCloseable_class_()]);
    }
    else {
      OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ output } count:1 type:JavaIoCloseable_class_()]);
    }
  }
}
