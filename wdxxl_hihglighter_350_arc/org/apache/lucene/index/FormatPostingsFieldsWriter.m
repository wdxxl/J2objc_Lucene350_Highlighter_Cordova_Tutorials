...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/FormatPostingsFieldsWriter.java
//
...
- (void)finish {
  OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ termsOut_, termsWriter_ } count:2 type:JavaIoCloseable_class_()]);
}
...
void OrgApacheLuceneIndexFormatPostingsFieldsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFieldInfos_(OrgApacheLuceneIndexFormatPostingsFieldsWriter *self, OrgApacheLuceneIndexSegmentWriteState *state, OrgApacheLuceneIndexFieldInfos *fieldInfos) {
  ...
  self->dir_ = ((OrgApacheLuceneIndexSegmentWriteState *) nil_chk(state))->directory_;
  self->segment_ = state->segmentName_;
  self->totalNumDocs_ = state->numDocs_;
  self->fieldInfos_ = fieldInfos;
  jboolean success = false;
  @try {
    self->termsOut_ = new_OrgApacheLuceneIndexTermInfosWriter_initWithOrgApacheLuceneStoreDirectory_withNSString_withOrgApacheLuceneIndexFieldInfos_withInt_(self->dir_, self->segment_, fieldInfos, state->termIndexInterval_);
    self->skipListWriter_ = new_OrgApacheLuceneIndexDefaultSkipListWriter_initWithInt_withInt_withInt_withOrgApacheLuceneStoreIndexOutput_withOrgApacheLuceneStoreIndexOutput_(self->termsOut_->skipInterval_, self->termsOut_->maxSkipLevels_, self->totalNumDocs_, nil, nil);
    self->termsWriter_ = new_OrgApacheLuceneIndexFormatPostingsTermsWriter_initWithOrgApacheLuceneIndexSegmentWriteState_withOrgApacheLuceneIndexFormatPostingsFieldsWriter_(state, self);
    success = true;
  }
  @finally {
    if (!success) {
      OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ self->termsOut_, self->termsWriter_ } count:2 type:JavaIoCloseable_class_()]);
    }
  }
}
