...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/NormsWriter.java
//
...
- (OrgApacheLuceneIndexInvertedDocEndConsumerPerThread *)addThreadWithOrgApacheLuceneIndexDocInverterPerThread:(OrgApacheLuceneIndexDocInverterPerThread *)docInverterPerThread {
  return new_OrgApacheLuceneIndexNormsWriterPerThread_initWithOrgApacheLuceneIndexDocInverterPerThread_withOrgApacheLuceneIndexNormsWriter_(docInverterPerThread, self);
}
...
withOrgApacheLuceneIndexSegmentWriteState:(OrgApacheLuceneIndexSegmentWriteState *)state {
  id<JavaUtilMap> byField = new_JavaUtilHashMap_init();
    ...
    while ([((id<JavaUtilIterator>) nil_chk(fieldsIt)) hasNext]) {
      ...
      if (((OrgApacheLuceneIndexNormsWriterPerField *) nil_chk(perField))->upto_ > 0) {
        ...
        if (l == nil) {
          ...
          (void) [byField putWithId:perField->fieldInfo_ withId:l];
        }
  ...
  @try {
    ...
    for (jint fieldNumber = 0; fieldNumber < numField; fieldNumber++) {
      ...
      if (toMerge != nil) {
        ...
        normCount++;
        IOSObjectArray *fields = [IOSObjectArray newArrayWithLength:numFields type:OrgApacheLuceneIndexNormsWriterPerField_class_()];
        IOSIntArray *uptos = [IOSIntArray newArrayWithLength:numFields];
        ...
        while (numLeft > 0) {
          ...
          if (IOSIntArray_Get(uptos, minLoc) == ((OrgApacheLuceneIndexNormsWriterPerField *) nil_chk(IOSObjectArray_Get(fields, minLoc)))->upto_) {
            ...
            if (minLoc != numLeft - 1) {
              (void) IOSObjectArray_Set(fields, minLoc, IOSObjectArray_Get(fields, numLeft - 1));
  ...
  @finally {
    if (success) {
      OrgApacheLuceneUtilIOUtils_closeWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ normsOut } count:1 type:JavaIoCloseable_class_()]);
    }
    else {
      OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ normsOut } count:1 type:JavaIoCloseable_class_()]);
    }
  }
}
