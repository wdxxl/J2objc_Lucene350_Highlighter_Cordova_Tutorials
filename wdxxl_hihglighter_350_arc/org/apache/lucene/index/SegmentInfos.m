...
//  source: jar:file:wdxxl_lucene_350_highlighter.jar!org/apache/lucene/index/SegmentInfos.java
//
...
- (id)java_clone {
  @try {
    ...
    ((OrgApacheLuceneIndexSegmentInfos *) nil_chk(sis))->segments_ = new_JavaUtilArrayList_initWithInt_([self size]);
    sis->segmentSet_ = new_JavaUtilHashSet_initWithInt_([self size]);
    sis->cachedUnmodifiableList_ = nil;
    sis->cachedUnmodifiableSet_ = nil;
    ...
    sis->userData_ = new_JavaUtilHashMap_initWithJavaUtilMap_(userData_);
    return sis;
  }
  @catch (JavaLangCloneNotSupportedException *e) {
    @throw new_JavaLangRuntimeException_initWithNSString_withNSException_(@"should not happen", e);
  }
}
...
- (id<JavaUtilCollection>)filesWithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)dir
  ...
  JavaUtilHashSet *files = new_JavaUtilHashSet_init();
...
- (NSString *)toStringWithOrgApacheLuceneStoreDirectory:(OrgApacheLuceneStoreDirectory *)directory {
  ...
  (void) [((JavaLangStringBuilder *) nil_chk([buffer appendWithNSString:[self getCurrentSegmentFileName]])) appendWithNSString:@": "];
  ...
  for (jint i = 0; i < count; i++) {
    ...
    OrgApacheLuceneIndexSegmentInfo *info = [self infoWithInt:i];
    (void) [buffer appendWithNSString:[((OrgApacheLuceneIndexSegmentInfo *) nil_chk(info)) toStringWithOrgApacheLuceneStoreDirectory:directory withInt:0]];
  }
...
- (void)setUserDataWithJavaUtilMap:(id<JavaUtilMap>)data {
  if (data == nil) {
    userData_ = JavaUtilCollections_emptyMap();
  }
  else {
    userData_ = data;
  }
...
- (void)applyMergeChangesWithOrgApacheLuceneIndexMergePolicy_OneMerge:(OrgApacheLuceneIndexMergePolicy_OneMerge *)merge
  ...
  id<JavaUtilSet> mergedAway = new_JavaUtilHashSet_initWithJavaUtilCollection_(((OrgApacheLuceneIndexMergePolicy_OneMerge *) nil_chk(merge))->segments_);
  ...
  for (jint segIdx = 0, cnt = [((id<JavaUtilList>) nil_chk(segments_)) size]; segIdx < cnt; segIdx++) {
    ...
    if ([mergedAway containsWithId:info]) {
      if (!inserted && !dropSegment) {
        (void) [((id<JavaUtilList>) nil_chk(segments_)) setWithInt:segIdx withId:merge->info_];
        inserted = true;
        newSegIdx++;
      }
    }
    else {
      (void) [((id<JavaUtilList>) nil_chk(segments_)) setWithInt:newSegIdx withId:info];
      newSegIdx++;
    }
  }
  if (!inserted && !dropSegment) {
...
- (id<JavaUtilList>)createBackupSegmentInfosWithBoolean:(jboolean)cloneChildren {
  if (cloneChildren) {
    id<JavaUtilList> list = new_JavaUtilArrayList_initWithInt_([self size]);
    for (OrgApacheLuceneIndexSegmentInfo * __strong info in self) {
    ...
    return new_JavaUtilArrayList_initWithJavaUtilCollection_(segments_);
  }
}
...
- (id<JavaUtilList>)asList {
  if (cachedUnmodifiableList_ == nil) {
    cachedUnmodifiableList_ = JavaUtilCollections_unmodifiableListWithJavaUtilList_(segments_);
  }
...
- (id<JavaUtilSet>)asSet {
  if (cachedUnmodifiableSet_ == nil) {
    cachedUnmodifiableSet_ = JavaUtilCollections_unmodifiableSetWithJavaUtilSet_(segmentSet_);
  }
...
- (void)addWithOrgApacheLuceneIndexSegmentInfo:(OrgApacheLuceneIndexSegmentInfo *)si {
  if ([((id<JavaUtilSet>) nil_chk(segmentSet_)) containsWithId:si]) {
    @throw new_JavaLangIllegalStateException_initWithNSString_(@"Cannot add the same segment two times to this SegmentInfos instance");
  }
...
+ (void)initialize {
  if (self == [OrgApacheLuceneIndexSegmentInfos class]) {
    OrgApacheLuceneIndexSegmentInfos_infoStream = nil;
...
void OrgApacheLuceneIndexSegmentInfos_init(OrgApacheLuceneIndexSegmentInfos *self) {
  ...
  self->userData_ = JavaUtilCollections_emptyMap();
  self->segments_ = new_JavaUtilArrayList_init();
  self->segmentSet_ = new_JavaUtilHashSet_init();
}
...
jlong OrgApacheLuceneIndexSegmentInfos_generationFromSegmentsFileNameWithNSString_(NSString *fileName) {
  ...
  if ([((NSString *) nil_chk(fileName)) isEqual:OrgApacheLuceneIndexIndexFileNames_SEGMENTS]) {
    ...
    @throw new_JavaLangIllegalArgumentException_initWithNSString_(JreStrcat("$$$", @"fileName \"", fileName, @"\" is not a segments file"));
  }
}
...
void OrgApacheLuceneIndexSegmentInfos_readWithOrgApacheLuceneStoreDirectory_withNSString_(OrgApacheLuceneIndexSegmentInfos *self, OrgApacheLuceneStoreDirectory *directory, NSString *segmentFileName) {
  ...
  [self clear];
  OrgApacheLuceneStoreChecksumIndexInput *input = new_OrgApacheLuceneStoreChecksumIndexInput_initWithOrgApacheLuceneStoreIndexInput_([((OrgApacheLuceneStoreDirectory *) nil_chk(directory)) openInputWithNSString:segmentFileName]);
  ...
  @try {
    ...
    if (format > OrgApacheLuceneIndexSegmentInfos_FORMAT_MINIMUM) {
      @throw new_OrgApacheLuceneIndexIndexFormatTooOldException_initWithOrgApacheLuceneStoreDataInput_withInt_withInt_withInt_(input, format, OrgApacheLuceneIndexSegmentInfos_FORMAT_MINIMUM, OrgApacheLuceneIndexSegmentInfos_FORMAT_MAXIMUM);
    }
    if (format < OrgApacheLuceneIndexSegmentInfos_FORMAT_MAXIMUM) {
      @throw new_OrgApacheLuceneIndexIndexFormatTooNewException_initWithOrgApacheLuceneStoreDataInput_withInt_withInt_withInt_(input, format, OrgApacheLuceneIndexSegmentInfos_FORMAT_MINIMUM, OrgApacheLuceneIndexSegmentInfos_FORMAT_MAXIMUM);
    }
    self->version__ = [input readLong];
    self->counter_ = [input readInt];
    for (jint i = [input readInt]; i > 0; i--) {
      OrgApacheLuceneIndexSegmentInfo *si = new_OrgApacheLuceneIndexSegmentInfo_initWithOrgApacheLuceneStoreDirectory_withInt_withOrgApacheLuceneStoreIndexInput_(directory, format, input);
      if ([si getVersion] == nil) {
        OrgApacheLuceneStoreDirectory *dir = directory;
        if ([si getDocStoreOffset] != -1) {
          if ([si getDocStoreIsCompoundFile]) {
            dir = new_OrgApacheLuceneIndexCompoundFileReader_initWithOrgApacheLuceneStoreDirectory_withNSString_withInt_(dir, OrgApacheLuceneIndexIndexFileNames_segmentFileNameWithNSString_withNSString_([si getDocStoreSegment], OrgApacheLuceneIndexIndexFileNames_COMPOUND_FILE_STORE_EXTENSION), 1024);
          }
        }
        else if ([si getUseCompoundFile]) {
          dir = new_OrgApacheLuceneIndexCompoundFileReader_initWithOrgApacheLuceneStoreDirectory_withNSString_withInt_(dir, OrgApacheLuceneIndexIndexFileNames_segmentFileNameWithNSString_withNSString_(si->name_, OrgApacheLuceneIndexIndexFileNames_COMPOUND_FILE_EXTENSION), 1024);
        }
        @try {
    ...
    if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_USER_DATA) {
      if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_DIAGNOSTICS) {
        self->userData_ = [input readStringStringMap];
      }
      else if (0 != [input readByte]) {
        self->userData_ = JavaUtilCollections_singletonMapWithId_withId_(@"userData", [input readString]);
      }
      else {
        self->userData_ = JavaUtilCollections_emptyMap();
      }
    ...
    if (format <= OrgApacheLuceneIndexSegmentInfos_FORMAT_CHECKSUM) {
      ...
      if (checksumNow != checksumThen) @throw new_OrgApacheLuceneIndexCorruptIndexException_initWithNSString_(JreStrcat("$@C", @"checksum mismatch in segments file (resource: ", input, ')'));
    }
    success = true;
  }
  @finally {
...
void OrgApacheLuceneIndexSegmentInfos_readWithOrgApacheLuceneStoreDirectory_(OrgApacheLuceneIndexSegmentInfos *self, OrgApacheLuceneStoreDirectory *directory) {
  ...
  (void) [new_OrgApacheLuceneIndexSegmentInfos_1_initWithOrgApacheLuceneIndexSegmentInfos_withOrgApacheLuceneStoreDirectory_(self, directory) run];
}
...
void OrgApacheLuceneIndexSegmentInfos_writeWithOrgApacheLuceneStoreDirectory_(OrgApacheLuceneIndexSegmentInfos *self, OrgApacheLuceneStoreDirectory *directory) {
  ...
  OrgApacheLuceneStoreChecksumIndexOutput *segnOutput = new_OrgApacheLuceneStoreChecksumIndexOutput_initWithOrgApacheLuceneStoreIndexOutput_([((OrgApacheLuceneStoreDirectory *) nil_chk(directory)) createOutputWithNSString:segmentFileName]);
  jboolean success = false;
  @try {
    ...
    self->pendingSegnOutput_ = segnOutput;
    success = true;
  }
  @finally {
    if (!success) {
      OrgApacheLuceneUtilIOUtils_closeWhileHandlingExceptionWithJavaIoCloseableArray_([IOSObjectArray newArrayWithObjects:(id[]){ segnOutput } count:1 type:JavaIoCloseable_class_()]);
      @try {
...
void OrgApacheLuceneIndexSegmentInfos_setInfoStreamWithJavaIoPrintStream_(JavaIoPrintStream *infoStream) {
  OrgApacheLuceneIndexSegmentInfos_initialize();
  OrgApacheLuceneIndexSegmentInfos_infoStream = infoStream;
}
...
void OrgApacheLuceneIndexSegmentInfos_prepareCommitWithOrgApacheLuceneStoreDirectory_(OrgApacheLuceneIndexSegmentInfos *self, OrgApacheLuceneStoreDirectory *dir) {
  if (self->pendingSegnOutput_ != nil) @throw new_JavaLangIllegalStateException_initWithNSString_(@"prepareCommit was already called");
...
void OrgApacheLuceneIndexSegmentInfos_finishCommitWithOrgApacheLuceneStoreDirectory_(OrgApacheLuceneIndexSegmentInfos *self, OrgApacheLuceneStoreDirectory *dir) {
  if (self->pendingSegnOutput_ == nil) @throw new_JavaLangIllegalStateException_initWithNSString_(@"prepareCommit was not called");
  jboolean success = false;
  @try {
...
- (id)runWithOrgApacheLuceneIndexIndexCommit:(OrgApacheLuceneIndexIndexCommit *)commit {
  if (commit != nil) {
    if (directory_ != [commit getDirectory]) @throw new_JavaIoIOException_initWithNSString_(@"the specified commit does not match the specified Directory");
  ...
  while (true) {
    if (useFirstMethod) {
      ...
      if (gen == -1) {
        @throw new_OrgApacheLuceneIndexIndexNotFoundException_initWithNSString_(JreStrcat("$@$$", @"no segments* file found in ", directory_, @": files: ", JavaUtilArrays_toStringWithNSObjectArray_(files)));
      }
    }
